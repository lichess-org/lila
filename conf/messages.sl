...
troll=Trolanje
other=Drugo
reportDescriptionHelp=Prilepi link do igre in obrazloži kaj je narobe s tem uporabnikom
by=od %s
thisTopicIsNowClosed=Tema je zaprta
theming=Nastavitve barve
donate=Doniraj
blog=Blog
map=Zemljevid
realTimeWorldMapOfChessMoves=Zemljevid potez v živo
questionsAndAnswers=Vprašanja & Odgovori
notes=Beležke
typePrivateNotesHere=Zasebne beležke
gameDisplay=Prikaz igre
pieceAnimation=Animacija figur
materialDifference=Razlika v materialu
closeAccount=Zapri račun
closeYourAccount=Zapri svoj račun
changedMindDoNotCloseAccount=Premislil sem si, nočem zapreti račun
closeAccountExplanation=Si prepričan, da želiš zapreti svoj račun? Zaprtje računa je končna odločitev. Tvoj račun ne bo več dodtopen.
thisAccountIsClosed=Ta račun je zaprt.
invalidUsernameOrPassword=Napačno uporabniško ime ali geslo
emailMeALink=Pošlji povezavo po Emailu
currentPassword=Trenutno geslo
newPassword=Novo geslo
newPasswordAgain=Novo geslo (ponovno)
boardHighlights=Osvetlitev sahovniče (zadnja poteza in šah)
pieceDestinations=Destinacije figur (regularne poteze in predpremiki potez)
...
never=Nikoli
whenTimeRemainingLessThanTenSeconds=Ko je preostanek časa < 10 sekund
horizontalGreenProgressBars=Zelena vodoravna prečka
soundWhenTimeGetsCritical=Zvok, ko čas postane kritičen
gameBehavior=Obnašanje igre
premovesPlayingDuringOpponentTurn=Predpremiki potez
takebacksWithOpponentApproval=Popravljanje potez (s soglasjem nasprotnika)
promoteToQueenAutomatically=Avtomatsko promoviraj v kraljico
claimDrawOnThreefoldRepetitionAutomatically=Avtomatsko razglasi remi ob %strikratni ponovitvi%s
privacy=Zasebnost
letOtherPlayersFollowYou=Omogoči da ti drugi sledijo
letOtherPlayersChallengeYou=Omogoči da te drugi izzovejo
sound=Zvok
soundControlInTheTopBarOfEveryPage=Nastavitev glasnosti je vedno zgoraj desno
...
always=Vedno
inCasualGamesOnly=Pri nerangiranih partijah
whenPremoving=V primeru predpremika poteze
whenTimeRemainingLessThanThirtySeconds=Če je ostalo < 30 sekund
difficultyEasy=Lahko
difficultyNormal=Normalno
difficultyHard=Težko
xLeftANoteOnY=%s pustil sporočilo %s
xCompetesInY=%s igra v %s
xAskedY=%s sprašuje v %s
xAnsweredY=%s je odgovoril v %s
xCommentedY=%s je komentiral v %s
timeline=Časovnica
seeAllTournaments=Poglej vse turnirje
starting=Začni turnir:
allInformationIsPublicAndOptional=Vsi podatki so javni in niso obvezni.
yourCityRegionOrDepartment=Tvoj kraj ali mesto.
biographyDescription=Povej o sebi, kaj imaš v šahu rad, najljubše otvoritve, partije, šahisti...
