...
createAGame=Utwórz partię
whiteIsVictorious=Białe wygrały
blackIsVictorious=Czarne wygrały
kingInTheCenter=Król w centrum
threeChecks=Trzy szachy
variantEnding=Wariant zakończony
playWithTheSameOpponentAgain=Zagraj jeszcze raz z tym przeciwnikiem
...
makeYourOpponentResign=Przyjmij, że przeciwnik się poddał
...
whiteCreatesTheGame=Białe utworzyły partię
blackCreatesTheGame=Czarne utworzyły partię
whiteJoinsTheGame=Pojawił się grający białymi
blackJoinsTheGame=Pojawił się grający czarnymi
...
shareThisUrlToLetSpectatorsSeeTheGame=Podziel się tym linkiem, aby zaprosić widzów do obserwowania tej partii
...
gamesBeingPlayedRightNow=Obecnie rozgrywane partie
viewAllNbGames=%s partii
viewNbCheckmates=%s matów
nbBookmarks=%s zakładek
nbPopularGames=%s popularnych partii
nbAnalysedGames=%s przeanalizowanych partii
bookmarkedByNbPlayers=Zakładki utworzone przez %s graczy
viewInFullSize=Zobacz pełny rozmiar
logOut=Wyloguj
signIn=Zaloguj się
...
nbGamesWithYou=%s partii z tobą
...
blackDeclinesDraw=Czarne odrzucają remisu
...
gameAborted=Partia została przerwana
standard=Standard
unlimited=Bez ograniczeń
mode=Tryb
casual=Towarzyska
rated=Rankingowa
thisGameIsRated=Ta partia będzie liczona do rankingu
rematch=Rewanż
...
pasteThePgnStringHere=Wklej tutaj partię w formacie PGN
fromPosition=Z pozycji
continueFromHere=Kontynuuj od tej pozycji
importGame=Importuj partię
nbImportedGames=%s zaimportowanych partii
...
colorPlaysCheckmateInOne=%s zaczynają i dają mata w jednym posunięciu
...
bewareTheGameIsRatedButHasNoClock=Uważaj, ta partia liczy się do rankingu, ale nie ma limitu czasu!
training=Trening
...
nbGamesInPlay=%s partii trwa
automaticallyProceedToNextGameAfterMoving=Automatycznie przejdź do następnej partii po wykonaniu ruchu
...
troll=Natręt (troll)
other=Inny powód
reportDescriptionHelp=Wklej link do partii i wytłumacz, dlaczego zachowanie użytkownika jest nieprawidłowe
by=przez %s
...
blog=Blog
map=Mapa
realTimeWorldMapOfChessMoves=Ruchy szachowe na mapie świata na żywo
questionsAndAnswers=Pytania i odpowiedzi
notes=Notatki
typePrivateNotesHere=Wpisz tu prywatną notatkę
...
gameBehavior=Zachowanie podczas partii
...
onSlowGames=W partiach granych dłuższym tempem
always=Zawsze
...
allowAnalytics=Wysyłaj anonimowe dane użycia
...
tournamentHomeTitle=Turnieje szachowe w różnych kontrolach czasu i odmianach szachów.
tournamentHomeDescription=Weź udział w turniejach szachowych online! Dołącz do istniejącego turnieju lub stwórz własny. Bullet, Blitz, Classical, Chess960, King of the Hill, Threecheck i wiele innych opcji, aby szachowa zabawa trwała w nieskończoność.
...
returnToTournamentsHomepage=Powrót na stronę turniejów
