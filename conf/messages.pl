...
makeYourOpponentResign=Wymuś zwycięstwo walkowerem
...
whiteCreatesTheGame=Białe utworzyły grę
...
games=Gry
forum=Forum
xPostedInForumY=%s napisał w temacie %s
...
rematchOfferAccepted=Oferta rewanżu została przyjęta
rematchOfferCanceled=Oferta rewanżu została anulowana
rematchOfferDeclined=Oferta rewanżu została odrzucona
cancelRematchOffer=Wycofaj ofertę rewanżu
...
takebackPropositionDeclined=Propozycja cofnięcia ruchu zostałaodrzucona
...
troll=Natręt (troll)
other=Inny powód
reportDescriptionHelp=Wklej link do tej gry(gier) i wytłumacz dlaczego zachowanie użytkownika jest nieprawidłowe
by=przez %s
...
allowAnalytics=Wysyłaj statystyki
...
blindfoldChess=Gra na ślepo (niewidzialne bierki)
