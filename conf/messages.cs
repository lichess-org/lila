...
troll=Troll
other=Jiné
reportDescriptionHelp=Vložte link na hru(y) a popište, co je špatně na chování tohoto hráče.
by=od %s
...
maximumNbCharacters=Machimum: %s znaků.
...
error.email_acceptable=Emailová adresa není přijmutelná
...
inCorrespondenceGames=V korespondenčních šachu
...
tournamentFAQ=Arena turnaje FAQ
tournamentOfficial=oficiální
timeBeforeTournamentStarts=Čas před začátkem turnaje
averageCentipawnLoss=Průměrná pěšcová ztráta
keyboardShortcuts=Klávesové zkratky
keyMoveBackwardOrForward=pohněte se dpředu/dozadu
...
keyYouCanDrawArrowsCirclesAndScrollToMove=shift a kliknutí nebo pravé tlačítko a kliknutí nakreslí kruhy a šipky na desku, Můžete také skrolovat přes desku pro pohyb ve hře
newTournament=Nový turnaj
tournamentHomeTitle=Šachový turnaj představuje různé časové kontroly a varianty
tournamentHomeDescription=Hrát rychle  šachový turnaj! Připojte se k oficiálnímu plánovanému turnaji, nebo si vytvořte vlastní. Bullet, Blitz, Klasické, Chess960(Fisherovky), Král v centru, Tři šachy, a vícero možností k dispozici pro nekonečnou šachovou zábavu.
...
weeklyPerfTypeRatingDistribution=Měsíční rozdělení %s hodnocení
nbPerfTypePlayersThisWeek=%s %s hráčů tento měsíc.
...
areYouSureYouEvenRegisteredYourEmailOnLichess=Jste si jistí, že jste se již dříve registrovali na lichess?
...
byRegisteringYouAgreeToBeBoundByOur=Registrací souhlasíte se zavázáním se k našim %s.
...
asFreeAsLichess=Svobodný stejně jako lichess
builtForTheLoveOfChessNotMoney=Vytvořeno z lásky k šachy nikoliv pro peníze
