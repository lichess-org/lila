...
signUp=Regístrate
computersAreNotAllowedToPlay=A las computadoras y los jugadores con ayuda de la computadora, no se les permite jugar. Por favor, no recibas ayuda de los motores de ajedrez, bases de datos o de otros jugadores durante el juego. También ten en cuenta que crear múltiples cuentas es un abuso y puede derivar en un bloqueo para tí
games=Partidas
forum=Foro
...
troll=Acoso
other=Otra
reportDescriptionHelp=Pegá el enlace a la(s) partida(s) y explicanos que hay de malo en el comportamiento de este usuario.
by=de %s
...
blog=Blog
map=Mapa
realTimeWorldMapOfChessMoves=Mapa mundial en tiempo real de movidas de ajedrez
...
blocks=%s bloqueado
listBlockedPlayers=Lista los jugadores que has bloqueado
human=Humano
computer=Ordenador
side=Color
clock=Reloj
unauthorizedError=Acceso no autorizado
noInternetConnection=No hay conexión a Internet. Aún puedes jugar offline desde el menú.
connectedToLichess=Ahora estás conectado a lichess.org
connectedToLichessAnonymous=Ahora tu estás conectado a lichess.org. Desde el menú puedes autenticarte o empezar un nuevo juego.
signedOut=Has sido correctamente desconectado.
playOnTheBoardOffline=Juega offline de manera local.
showPGN=Muestra registro PGN
