...
weHaveSentYouAnEmailClickTheLink=Te hemos enviado un correo. Click al enlace que se encuentra dentro del correo para activar tu cuenta.
ifYouDoNotSeeTheEmailCheckOtherPlaces=Si no ves el correo, verifica otros lugares donde podría estar, como la papelera, spam, social u otras carpetas.
areYouSureYouEvenRegisteredYourEmailOnLichess=¿ Estás seguro que registastre tu correo en lichess ?
itWasNotRequiredForYourRegistration=No es requerido para tu registro.
weHaveSentYouAnEmailTo=Te hemos enviado un correo a %s. Click al enlace dentro del correo para resetear tu contraseña.
byRegisteringYouAgreeToBeBoundByOur=Al registrarte, tu estás de acuerdo ser vinculado por nuestro %s.
networkLagBetweenYouAndLichess=Retraso de la red entre lichess y tú
timeToProcessAMoveOnLichessServer=Tiempo que el servidor de lichess procesa un movimiento
downloadAnnotated=Descargar notación
downloadRaw=Descarga
downloadImported=Descarga importada
printFriendlyPDF=Descargar en PDF
crosstable=Tabla del match
youCanAlsoScrollOverTheBoardToMoveInTheGame=TU también puedes usar la rueda del mouse para hacer movimientos de la partida.
