...
db.relay.find({ 'sync.upstream.lcc': { $exists: 1 } }).sort({ $natural: -1 }).limit(30).forEach(relay => {

  db.relay.updateOne({ _id: relay._id }, {
    $set: {
...
db.relay.find({ 'sync.upstream.urls': { $exists: 1 } }).sort({ $natural: -1 }).limit(30).forEach(relay => {
