...
trait AssetHelper extends HasEnv { self: I18nHelper with SecurityHelper =>
  ...
  def cssTag(name: String)(using ctx: Context): Frag =
    cssTagWithDir(name, isRTL(using ctx.lang))

  def cssTagWithDir(name: String, isRTL: Boolean): Frag =
    cssAt(s"css/$name.${if (isRTL) "rtl" else "ltr"}.${if (minifiedAssets) "min" else "dev"}.css")
