...
object list:
  ...
  private def tournamentWinners(winners: List[lila.tournament.Winner])(using PageContext) =
  ...
  private def userTopPerf(users: List[User.LightPerf], perfType: PerfType)(using ctx: PageContext) =
    st.section(cls := "user-top")(
