...
object mod {
    ...
    div(id := "mz_actions")(
      ...
      div(cls := "btn-rack")(
        isGranted(_.IpBan) option {
          st.form(method := "POST", action := routes.Mod.ban(u.username, !u.ipBan), title := "Bans all IPs under this account from logging into this site.", cls := "xhr")(
      ...
      isGranted(_.SetTitle) option {
        st.form(cls := "fmjd_title", method := "POST", action := routes.Mod.setTitle(u.username))(
  ...
  def assessments(pag: lidraughts.evaluation.PlayerAggregateAssessment.WithGames)(implicit ctx: Context) = div(id := "mz_assessments")(
  ...
  def otherUsers(u: User, spy: lidraughts.security.UserSpy, notes: List[lidraughts.user.Note], bans: Map[String, Int])(implicit ctx: Context) = div(id := "mz_others")(
    table(cls := "slist")(
      thead(
        tr(
  ...
  def identification(u: User, spy: lidraughts.security.UserSpy)(implicit ctx: Context) = div(id := "mz_identification")(
    div(cls := "spy_ips")(
