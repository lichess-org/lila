...
object download:
  def apply(user: lila.user.User)(implicit ctx: WebContext): Frag =
  ...
  private def color(implicit ctx: WebContext): Frag = tr(
  ...
  private def date(implicit ctx: WebContext): Frag = tr(
  ...
  private def opponent(implicit ctx: WebContext): Frag = tr(
    th(label(`for` := "dl-opponent")(trans.search.opponentName())),
    td(input(tpe := "text", id := "dl-opponent", name := "vs"))
  )

  private def mode(implicit ctx: WebContext): Frag = tr(
  ...
  private def analysis(implicit ctx: WebContext): Frag = tr(
    th(
  ...
  private def perfToggles(implicit ctx: WebContext): Frag =
  ...
  private def perfToggle(perfType: lila.rating.PerfType)(implicit ctx: WebContext): Frag = div(
    form3.cmnToggle(
  ...
  private def includeToggles(implicit ctx: WebContext): Frag = tr(
    th(cls := "top")(
  ...
  private def amount(implicit ctx: WebContext): Frag = tr(
    th(
      label(`for` := "dl-amount")(
