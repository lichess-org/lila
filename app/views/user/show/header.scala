  ...
  def apply(
    ...
    div(cls := "social content_box_inter")(
      div(cls := "links")(
        ...
        a(href := routes.Study.byOwnerDefault(u.username), cls := "intertab")(
        ...
        (ctx.isAuth && ctx.noKid && !ctx.is(u)) option
          a(cls := "intertab note_zone_toggle")(splitNumberUnsafe(social.notes.size + " Notes")),
        div(cls := "user_actions")(
          (ctx is u) option frag(
            a(cls := "button", href := routes.Account.profile, title := trans.editProfile.txt(), dataIcon := "%"),
            a(cls := "button", href := routes.Relation.blocks(), title := trans.listBlockedPlayers.txt(), dataIcon := "k")
          ),
            ...
            div(cls := "user-infos scroll-shadow-hard")(
              if (!ctx.is(u)) frag(
                u.engine option div(cls := "warning engine_warning")(
                  ...
                  trans.thisPlayerUsesDraughtsComputerAssistance.frag()
                ),
...
It's useful against spambots. These marks are not visible to the public."""
              ...
              else u.title.flatMap(lidraughts.user.Title.names.get).map { title =>
      ...
      span(dataTab := "other", cls := List("intertab" -> true, "active" -> (angle == Angle.Other)))
    )
  )
}
