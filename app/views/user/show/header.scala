...
def apply(
  ...
  social: lidraughts.app.mashup.UserInfo.Social)(implicit ctx: Context) =
div(cls := "box__top")(
...
<div id="us_profile" cls := "@if(info.insightVisible){ with_insights}">
  ...
  <div cls := "user-infos scroll-shadow-hard">
    ...
    @u.title.flatMap(lidraughts.user.Title.names.get).map { title =>
  ...
  <a cls := "insight" href="@routes.Insight.index(u.username)">
    ...
    <strong>Draughts Insights</strong>
...
<div cls := "content_box_inter angles tabs">
  ...
  @defining(lidraughts.app.mashup.GameFilter.All) { f =>
