      ...
      div(cls := "tour__form box box-pad")(
        h1("Edit ", tour.fullName,
          st.form(cls := "form3", action := routes.Tournament.update(tour.id), method := "POST")(
            ...
            form3.group(form("position"), trans.startPosition(), klass = "position")(views.html.tournament.form.startingPosition(_)),
            form3.split(
            ...
            form3.group(form("description"), trans.tournamentDescription(), help = trans.tournamentDescriptionHelp().some)(form3.textarea(_)(rows := 2)),
          ...
          st.form(cls := "terminate", action := routes.Tournament.terminate(tour.id), method := "POST")(
            form3.submit(trans.cancelTheTournament(), icon = "j".some, klass = "text button button-red confirm")
          ))
      )
    ))
}