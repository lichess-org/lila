...
object crud {
  ...
  def create(form: Form[_])(implicit ctx: Context) = layout(
    ...
    div(cls := "crud page-menu__content box box-pad")(
      h1("New tournament"),
      st.form(cls := "content_box_content form3", action := routes.TournamentCrud.create, method := "POST")(inForm(form))
    )
  }
  ...
  def edit(tour: Tournament, form: Form[_])(implicit ctx: Context) = layout(
    ...
    div(cls := "crud edit page-menu__content box box-pad")(
      ...
      st.form(cls := "content_box_content form3", action := routes.TournamentCrud.update(tour.id), method := "POST")(inForm(form))
    )
  }
