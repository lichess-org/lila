...
object index {
  ...
  def apply(
    ...
    val primaryPost =
      (pager.currentPage == 1).??(pager.currentPageResults.headOption) flatMap FullPost
        .fromDocument("blog")
        ...
        div(cls := "blog index page-menu__content page-small box")(
          ...
          div(cls := "blog-cards list infinitescroll")(
            pager.currentPageResults flatMap MiniPost.fromDocument("blog", "wide") map { post =>
              primaryPost
                .fold(true)(_.id != post.id) option bits.postCard(post, "paginated".some, h3)
            },
