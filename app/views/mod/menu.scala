...
object menu:
  ...
  def apply(active: String)(using PageContext) =
    views.html.base.bits.pageMenuSubnav(
      ...
      isGranted(_.GamifyView).option(a(cls := active.active("log"), href := routes.Mod.log)("My logs")),
      isGranted(_.UserSearch)
        .option(a(cls := active.active("search"), href := routes.Mod.search)("Search users")),
      isGranted(_.Admin).option(a(cls := active.active("notes"), href := routes.Mod.notes())("Mod notes")),
      isGranted(_.SetEmail)
        .option(a(cls := active.active("email"), href := routes.Mod.emailConfirm)("Email confirm")),
      isGranted(_.Pages).option(a(cls := active.active("cms"), href := routes.Cms.index)("Pages")),
      isGranted(_.PracticeConfig)
      ...
      isGranted(_.Settings)
        .option(a(cls := active.active("setting"), href := routes.Dev.settings)("Settings")),
      isGranted(_.Cli).option(a(cls := active.active("cli"), href := routes.Dev.cli)("CLI"))
    )
