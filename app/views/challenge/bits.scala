...
object bits {
  ...
  def js(c: lidraughts.challenge.Challenge, json: play.api.libs.json.JsObject, owner: Boolean)(implicit ctx: Context) =
    frag(
...
data: ${safeJsonValue(json)}
  ...
  def explanation(c: lidraughts.challenge.Challenge)(implicit ctx: Context) = p(
