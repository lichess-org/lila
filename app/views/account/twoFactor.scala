...
object twoFactor:
  ...
  def setup(u: lila.user.User, form: play.api.data.Form[?])(implicit ctx: WebContext) =
    account.layout(
  ...
  def disable(u: lila.user.User, form: play.api.data.Form[?])(implicit ctx: WebContext) =
    account.layout(
      title = s"${u.username} - ${twoFactorAuth.txt()}",
      active = "twofactor"
    ) {
