...
object ask {
  ...
  def renderInner(ask: Ask, prevView: Option[List[Int]] = None, tally: Boolean = false)(using
    ...
    fieldset(
      ...
      ask.isConcluded option label(
        if (ask.choices.nonEmpty) s"${ask.picks ?? (_ size)} responses"
  ...
  private def pollBody(ask: Ask, view: List[Int])(using ctx: Context): Frag = frag {
    choiceContainer(ask) {
  ...
  private def rankBody(ask: Ask, view: List[Int])(using ctx: Context): Seq[Frag] = Seq[Frag](
    choiceContainer(ask)(
  ...
  private def quizBody(ask: Ask, view: List[Int])(using ctx: Context): Frag = {
  ...
  def barGraphBody(ask: Ask)(using Context): Frag =
    div(cls := "ask__graph")(frag {
      ...
      val max    = totals.max
      totals.zipWithIndex flatMap { case (total, choice) =>
        val pct  = if (max == 0) 0 else total * 100 / max
  ...
  private def whoPicked(ask: Ask, choice: Int, max: Int = 40): String = {
    ...
    if (ask.isAnon) s"${who.size} anonymous users"
    else
  ...
  sealed abstract class RenderType() // TODO - fix this scala fuckery
  object RenderType {
