...
object bits {
  ...
  private[video] def card(vv: lila.video.VideoView, control: lila.video.UserControl) =
    a(
      cls  := "card paginated",
      href := s"${routes.Video.show(vv.video.id)}?${control.queryStringUnlessBot}",
    )(
  ...
  def author(
      name: String,
      videos: Paginator[lila.video.VideoView],
      ...
      div(cls := "list infinitescroll box__pad")(
        ...
        videos.nextPage.map { next =>
          div(cls := "pager none")(
            a(
              rel  := "next",
              href := s"${routes.Video.author(name)}?${control.queryString}&page=${next}",
            )(
              "Next",
            ),
          )
