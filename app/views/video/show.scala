      ...
      openGraph = lila.app.ui.OpenGraph(
        ...
        div(cls := "show")(
          ...
          div(cls := "meta box__pad")(
            ...
            a(cls := "author", href := s"${routes.Video.author(video.author)}?${control.queryString}")(video.author),
            video.tags.map { tag =>
              a(cls := "tag", dataIcon := "o", href := s"${routes.Video.index}?tags=${tag.replace(" ", "+")}")(tag.capitalize)
            },
