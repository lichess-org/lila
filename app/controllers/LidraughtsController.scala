...
private[controllers] trait LidraughtsController
  ...
  protected def ensureSessionId(req: RequestHeader)(res: Result): Result =
    req.session.data.contains(LidraughtsCookie.sessionId).fold(
      res,
      res withCookies LidraughtsCookie.makeSessionId(req)
    )

  protected def negotiate(html: => Fu[Result], api: ApiVersion => Fu[Result])(implicit ctx: Context): Fu[Result] =
    (lidraughts.api.Mobile.Api.requestVersion(ctx.req) match {
  ...
  private def pageDataBuilder(ctx: UserContext, hasFingerprint: Boolean): Fu[PageData] = {
    ...
    ctx.me.fold(fuccess(PageData.anon(ctx.req, getAssetVersion, Nonce forRequest ctx.req, blindMode(ctx)))) { me =>
      ...
      Env.pref.api.getPref(me, ctx.req) zip {
        ...
        case (pref, (onlineFriends ~ teamNbRequests ~ nbChallenges ~ nbNotifications ~ inquiry)) =>
          PageData(onlineFriends, teamNbRequests, nbChallenges, nbNotifications, pref,
            ...
            inquiry = inquiry,
            nonce = Nonce.forRequest(ctx.req))
      }
    }
  }
  ...
  protected def RequireHttp11(result: => Fu[Result])(implicit ctx: lidraughts.api.Context): Fu[Result] =
