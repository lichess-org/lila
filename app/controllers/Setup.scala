                ...
                AnonHookRateLimit(req.ipAddress, rateLimited, cost = ctx.isAnon.so(1)):
                  for
                    ...
                    res <- processor.hook(
                      ...
                      sri,
                      req.sid,
                      lila.hub.pool.Blocking(blocking)
                    )(using me)
          ...
          Found(env.game.gameRepo.game(gameId)): game =>
            for
              ...
              hookConfigWithRating = get("rr")
                ...
                .updateFrom(game)
              allBlocking = lila.hub.pool.Blocking(blocking ++ game.userIds)
            ...
            .fold(
              ...
              config =>
                for
                  ...
                  res <- config.fixColor
                    .hook(reqSri | Sri(uniqId), me, sid = uniqId.some, lila.hub.pool.Blocking(blocking))
                    .match
                      case Left(hook) =>
                        PostRateLimit(req.ipAddress, rateLimited):
