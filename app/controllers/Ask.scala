...
final class Ask(env: Env) extends LilaController(env) {
  ...
  def view(aid: String, view: Option[String], tally: Boolean) =
    Open { implicit ctx =>
  ...
  def feedback(aid: String, view: Option[String], anon: Boolean) =
    AuthBody { implicit ctx => me =>
      implicit val req = ctx.body
      env.ask.api.setFeedback(
        aid,
        if anon then lila.ask.Ask.anon(me.id, aid) else me.id,
  ...
  def unset(aid: String, view: Option[String], anon: Boolean) =
    AuthBody { implicit ctx => me =>
      env.ask.api.unset(aid, if anon then lila.ask.Ask.anon(me.id, aid) else me.id) map {
        case Some(ask) => Ok(views.html.ask.renderInner(ask, paramToList(view)))
        case None      => NotFound(s"Ask $aid not found")
      }
    }
