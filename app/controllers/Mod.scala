...
object Mod extends LidraughtsController {
  ...
  def warn(username: String, subject: String) = OAuthModBody(_.ModMessage) { me =>
    lidraughts.message.ModPreset.bySubject(subject) ?? { preset =>
      withSuspect(username) { prev =>
        for {
          inquiry <- Env.report.api.inquiries ofModId me.id
  ...
  def search = SecureBody(_.UserSearch) { implicit ctx => me =>
    implicit def req = ctx.body
    val f = lidraughts.mod.UserSearch.form
    ...
    get("q") match {
      ...
      case Some(rawQuery) =>
        ...
        def tryWith(setEmail: EmailAddress, q: String): Fu[Option[Result]] =
          Env.mod.search(lidraughts.mod.UserSearch.exact(q)) flatMap {
  ...
  def cheatList(gameId: String) = SecureBody(_.Hunter) { implicit ctx => me =>
    OptionFuOk(lidraughts.game.GameRepo game gameId) { game =>
      Env.mod.cheatList.set(game, getBool("v"), lidraughts.report.Mod(me))
    }
  }
