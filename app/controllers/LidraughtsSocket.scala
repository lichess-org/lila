...
trait LidraughtsSocket { self: LidraughtsController =>
  ...
  protected def SocketOptionLimited[A: FrameFormatter](limiter: lidraughts.memo.RateLimit[IpAddress], name: String)(f: Context => Fu[Option[Pipe[A]]]) =
  ...
  private val rateLimitLogger = lidraughts.log("ratelimit")

  private def rateLimitedSocket[A: FrameFormatter](limiter: lidraughts.memo.RateLimit[IpAddress], name: String)(f: AcceptType[A]): WebSocket[A, A] =
    WebSocket[A, A] { req =>
      SocketCSRF(req) {
