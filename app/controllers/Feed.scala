  ...
  def index(page: Int) = Open: ctx ?=>
    Reasonable(page):
      for
        ...
        asks <- env.ask.repo.asksIn(updates.currentPageResults.map(_.content.value)*)
        renderedPage <- renderPage(views.feed.index(updates).copy(askFrags = asks.map(views.askUi.render)))
