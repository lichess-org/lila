  ...
  var l = function (e, t) {
    ...
    y = function () {
      ...
      try {
        ...
        u &&
          (c.push(
            "webgl unmasked vendor:" + o.getParameter(u.UNMASKED_VENDOR_WEBGL)
          ),
          c.push(
            "webgl unmasked renderer:" +
              o.getParameter(u.UNMASKED_RENDERER_WEBGL)
          ));
      } catch (s) {}
      return (
    ...
    _ = function () {
      ...
      if (void 0 !== t) {
        if (
          ...
          return !0;
        if (0 <= t.indexOf("linux") && "Linux" !== r && "Android" !== r)
          return !0;
    ...
    k = [
      ...
      },
      {
        ...
        getData: function (e, t) {
          var d = ["monospace", "sans-serif", "serif"],
            ...
            o = function (e) {
              for (var t = !1, n = 0; n < d.length; n++)
                if (
                  (t =
                    e[n].offsetWidth !== r[d[n]] ||
                    e[n].offsetHeight !== i[d[n]])
                )
                  return t;
              return t;
            },
          ...
          n.appendChild(f);
          for (var m = [], p = 0, A = l.length; p < A; p++)
            o(h[l[p]]) && m.push(l[p]);
...
(window.requestIdleCallback || window.setTimeout).bind(window)(() => {
  ...
  const storage = lishogi.storage.make("fipr");
  const send = (hash) => {
    storage.set(hash);
    const $i = $("#signup-fp-input");
    if ($i.length) $i.val(hash);
    else
      $.post("/auth/set-fp/" + hash + "/" + Math.round(performance.now() - t));
  };
  if (storage.get()) send(storage.get());
  else
    fipr.get((c) => send(fipr.x64hash128(c.map((x) => x.value).join(""), 31)));
});
