...
- These icons are mostly applied as masks, so all color values save alpha are ignored. Color is controlled by the host element `color` style.
- Alpha/`fill-opacity`/`stroke-opacity` affects the mask strength.
