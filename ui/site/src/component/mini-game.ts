  ...
  if (!window.Chessground) setTimeout(() => init(node), 200);
    ...
    const [fen, orientation, lm] = node.getAttribute("data-state")!.split(","),
      config = {
        ...
        lastMove:
          lm &&
          (lm[1] === "@" ? [lm.slice(2)] : [lm[0] + lm[1], lm[2] + lm[3]]),
        drawable: {
          enabled: false,
          visible: false,
        },
    ...
    domData.set(
      $cg[0] as HTMLElement,
      "chessground",
      window.Chessground($cg[0], config),
    );
...
export const initAll = (parent?: HTMLElement) => {
  const nodes = Array.from(
      (parent || document).getElementsByClassName("mini-game--init"),
    ),
    ids = nodes.map(init).filter(id => id);
  if (ids.length)
    lichess.StrongSocket.firstConnect.then(send =>
