...
lichess.load.then(() => {
  ...
  var todo = (function () {
    ...
    var must = [
      ...
      },
      {
        ...
        check() {
          return (
            $editor
              .find(".panel.texts textarea")
              .filter(function (this: HTMLTextAreaElement) {
                return !!$(this).val();
              }).length >= 3
          );
        },
    ...
    return function () {
      const points: Cash[] = must
        .filter(o => !o.check())
        .map(o => $("<li>").html(o.html));
  ...
  $reviews.find(".actions a").on("click", function (this: HTMLAnchorElement) {
    ...
    xhr.text($review.data("action") + "?v=" + $(this).data("value"), {
      method: "post",
    });
    $review.hide();
    $editor
      .find(".tabs div[data-tab=reviews]")
      .data("count", $reviews.find(".review").length - 1);
    return false;
  });

  $(".coach_picture form.upload input[type=file]").on(
    "change",
