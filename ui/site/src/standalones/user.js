$(function () {
  ...
  if (
    $("#perfStat.correspondence .view_games").length &&
    lishogi.once("user-correspondence-view-games")
  )
  ...
  $(".user-show .angles").each(function () {
    ...
    function browseTo(path) {
      ...
      $.get(path).then(function (html) {
        ...
        lishogi.loadInfiniteScroll(".angle-content .infinitescroll");
      });
    }
    ...
    $(".user-show").on("click", "#games a", function () {
      if (
        $("#games .to-search").hasClass("active") ||
        $(this).hasClass("to-search")
      )
        return true;
      $(this).addClass("active");
      browseTo($(this).attr("href"));
