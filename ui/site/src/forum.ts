...
lichess.load.then(() => {
  $(".forum")
    ...
    .on("click", "form.unsub button", function (this: HTMLButtonElement) {
      ...
      xhr.text(`${form.action}?unsub=${$(this).data("unsub")}`, {
        method: "post",
      });
      return false;
    });
  ...
  $(".post-text-area").one("focus", function (this: HTMLTextAreaElement) {
    ...
    if (topicId)
      lichess.loadScript("vendor/textcomplete.min.js").then(function () {
        ...
        const textcomplete = new window.Textcomplete(
          new window.Textcomplete.editors.Textarea(textarea),
        );
              ...
              search: function (term, callback) {
                ...
                threadParticipants.then(function (participants) {
                  ...
                  if (forumParticipantCandidates.length != 0) {
                    ...
                    xhr
                      .json(xhr.url("/player/autocomplete", { term }), {
                        cache: "default",
                      })
                      .then(candidateUsers =>
