...
export function boardSetting(): Setting<BoardStyle> {
  return makeSetting<BoardStyle>({
    choices: [
      ...
      [
        "table",
        "table: layout using a table with rank and file columns and row headers",
      ],
    ],
    default: "plain",
...
export function lastCaptured(
  ...
  for (var p of "kKqQrRbBnNpP") {
    const diff =
      oldSplitFen.split(p).length - 1 - (newSplitFen.split(p).length - 1);
...
export function renderBoard(
  ...
  const doPiece = (rank: Rank, file: File): VNode => {
    ...
    if (piece) {
      ...
      const pieceText = renderPieceStyle(
        piece.color === "white" ? role.toUpperCase() : role,
        pieceStyle,
      );
      ...
      return h(
        pieceWrapper,
        doPieceButton(rank, file, role, piece.color, text),
      );
    } else {
  ...
  return h(
    boardStyle === "table" ? "table.board-wrapper" : "div.board-wrapper",
    ranks,
  );
}
  ...
  return (ev: KeyboardEvent) => {
    ...
    if ($currBtn.attr("promotion") === "true") {
      ...
      if (!$promotionPiece.match(/^[qnrb]$/)) {
        $boardLive.text(
          "Invalid promotion piece. q for queen, n for knight, r for rook, b for bisho",
        );
        return false;
      }
...
export function selectionHandler(
  ...
  return (ev: MouseEvent) => {
    ...
    if ($moveBox.val() === "") {
      ...
      if (
        $evBtn.attr("color") === (opponentColor === "black" ? "white" : "black")
      )
        return false;

      const $first = $moveBox.val();
      const $firstPiece = $(
        '.board-wrapper [file="' + $first[0] + '"][rank="' + $first[1] + '"]',
      );
...
export function boardCommandsHandler() {
  return (ev: KeyboardEvent) => {
    ...
    const $boardLive = $(".boardstatus");
    const $position =
      ($currBtn.attr("file") ?? "") + ($currBtn.attr("rank") ?? "");
    if (ev.key === "o") {
      $boardLive.text();
...
export function possibleMovesHandler(
  ...
  return (ev: KeyboardEvent) => {
    ...
    const possibleMoves = chessgroundDests(
      Chess.fromSetup(
        parseFen(
          fen().replace(" " + opponentTurnFen + " ", " " + myTurnFen + " "),
        ).unwrap(),
      ).unwrap(),
    )
      .get($pos)
