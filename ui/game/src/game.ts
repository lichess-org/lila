...
export const isPlayerPlaying = (data: GameData): boolean =>
  playable(data) && !data.player.spectator;
...
export const mandatory = (data: GameData): boolean =>
  !!data.tournament || !!data.simul || !!data.swiss;

export const playedTurns = (data: GameData): number =>
  data.game.turns - (data.game.startedAtTurn || 0);

export const bothPlayersHavePlayed = (data: GameData): boolean =>
  playedTurns(data) > 1;
