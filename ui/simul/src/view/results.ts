...
export default function(ctrl: SimulCtrl) {
  return h('div.results', [
    h('div', trans(ctrl, 'nbPlaying', p => p.game.status < status.ids.mate)),
    h('div', trans(ctrl, 'nbWins', p => p.game.winner === p.hostColor)),
    h('div', trans(ctrl, 'nbDraws', p => p.game.status >= status.ids.mate && !p.game.winner)),
...
const splitNumber = (s: string) => {
  ...
  if ((found = s.match(NumberFirstRegex))) return [
    h('div.number', found[1]),
    h('div.text', found[2])
  ];
  if ((found = s.match(NumberLastRegex))) return [
    h('div.number', found[2]),
    h('div.text', found[1])
  ];
  return h('div.text', s);
}
