  ...
  return h("span.mini-game__player", [
    ...
    clock
      ...
      : h(
          ...
          game.winner ? (game.winner == color ? 1 : 0) : "½",
        ),
  ]);
}
...
const initMiniGame = (node: VNode) =>
  lichess.miniGame.initAll(node.elm as HTMLElement);
      ...
      ctrl.data.duels.length
        ? h(
            ...
            [h("h2", "Top games")].concat(
              ctrl.data.duels.map(
                renderDuel(ctrl.data.teamBattle, ctrl.data.duelTeams),
              ),
            ),
          )
        : null,
