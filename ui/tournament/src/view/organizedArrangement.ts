...
import { colors } from 'shogiground/types';
...
const organizerArrangementForm = (ctrl: TournamentController): MaybeVNode => {
  ...
  const user1Id = state.user1?.id,
    ...
    gamesBetweenUsers =
      user1Id && user2Id
        ? ctrl.data.standing.arrangements.filter(a => arrangementHasUser(a, user1Id) && arrangementHasUser(a, user2Id))
        : [];

  const updateState = <K extends keyof NewArrangement>(key: K, value: NewArrangement[K], redraw = false) => {
    state[key] = value;
      ...
      h('div.sides.color-wrap', [
        h(
          'select',
          {
            ...
            on: {
              change: (e: Event) =>
                updateState('color', (e.target as HTMLInputElement).value as Color | undefined, true),
            },
          },
          colorOptions(ctrl, state.color, false)
        ),
        h(
          'select',
          {
          ...
          colorOptions(ctrl, state.color, true)
        ),
      ]),
    ]),
...
function colorOptions(ctrl: TournamentController, stateColor: Color | undefined, flip: boolean) {
  return [
    h('option', { attrs: { value: '', selected: !stateColor } }, ctrl.trans.noarg('randomColor')),
    ...colors.map(color => {
      const realColor = flip ? opposite(color) : color;
      return h('option', { attrs: { value: realColor, selected: stateColor === realColor } }, ctrl.trans.noarg(color));
    }),
  ];
}
...
function playerOptions(
  ...
  return [
    ...
    ...ctrl.data.standing.players.map(player => {
      return h(
        'option',
        { attrs: { value: player.id, selected: player.id === selectedUserId, disabled: player.id === otherUserId } },
        player.name
      );
    }),
  ];
