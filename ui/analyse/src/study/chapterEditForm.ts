        ...
        content: [
          ...
          h(
            "form.form3",
            {
              hook: bindSubmit(e => {
                const o: any = {};
                "name mode orientation description"
                  .split(" ")
                  .forEach(field => {
          ...
          h("div.destructive", [
            h(
              emptyRedButton,
              {
                hook: bind("click", _ => {
                  if (
                    confirm(ctrl.trans.noarg("clearAllCommentsInThisChapter"))
                  )
...
function isLoaded(
  data: StudyChapterMeta | StudyChapterConfig,
): data is StudyChapterConfig {
  return data.hasOwnProperty("orientation");
}

function viewLoaded(
  ctrl: StudyChapterEditFormCtrl,
  data: StudyChapterConfig,
): VNode[] {
