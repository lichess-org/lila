...
function toolButton(opts: ToolButtonOpts): VNode {
  return h(
    ...
    [
      opts.count
        ? h("count.data-count", { attrs: { "data-count": opts.count } })
        : null,
      opts.icon,
    ],
  );
...
function buttons(root: AnalyseCtrl): VNode {
  const ctrl: StudyCtrl = root.study!,
    ...
    showSticky =
      ctrl.data.features.sticky &&
      (canContribute || (ctrl.vm.behind && ctrl.isUpdatedRecently())),
  ...
  return h("div.study__metadata", [
    h("h2", [
      h("span.name", { attrs: { title } }, [
        title,
        credit
          ? h("span.credit", { hook: richHTML(credit, false) })
          : undefined,
      ]),
      h(
        "span.liking.text",
...
export function overboard(ctrl: StudyCtrl) {
  ...
  if (ctrl.chapters.editForm.current())
    return chapterEditFormView(ctrl.chapters.editForm);
  if (ctrl.members.inviteForm.open())
    return inviteFormView(ctrl.members.inviteForm);
