...
import { type VNode, type LooseVNode, onInsert, iconTag, bind, dataIcon, looseH as h } from 'lib/snabbdom';
...
export function studySideNodes(ctrl: StudyCtrl, withSearch: boolean): LooseVNode[] {
  ...
  return [
    h('div.study__side', [tabs, content, ...(displayColumns() > 2 ? trailer : [])]),
...
export function underboard(ctrl: AnalyseCtrl): LooseVNode[] {
  ...
  switch (toolTab) {
    ...
    case 'serverEval':
      ...
      if (study?.relay) panel = h('div.eval-chart-and-training', [panel, trainingView(ctrl)]);
      break;
    case 'share':
