import { isFiftyMoves } from 'lib/game/chess';
...
export default function (root: AnalyseCtrl, goal: Goal, nbMoves: number): boolean | null {
  ...
  switch (goal.result) {
    case 'drawIn':
    case 'equalIn':
      if (node.threefold || isFiftyMoves(root.variantKey, node.fen)) return true;
    ...
    case 'mate':
      if (node.threefold || isFiftyMoves(root.variantKey, node.fen)) return false;
