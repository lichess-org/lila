...
export const throttled = (sound: string) =>
  throttle(100, () => lichess.sound.play(sound));
            ...
            "div.board",
            {
              hook: {
                insert: el => {
                  ...
                  $buttons.on(
                    "keydown",
                    arrowKeyHandler(ctrl.data.player.color, borderSound),
                  );
                  $buttons.on("keypress", positionJumpHandler());
                  $buttons.on(
                    "keypress",
                    pieceJumpingHandler(wrapSound, errorSound),
                  );
                },
              },
            },
