  ...
  return h("div.player.off", [
    ...
    h("div.instruction", [
      ...
      h("div.choices", [
        h(
          "a",
          { hook: bind("click", ctrl.resume, ctrl.redraw) },
          root.trans.noarg("resumePractice"),
        ),
      ]),
    ]),
  ]);
      ...
      h("strong", root.trans.noarg(end.winner ? "checkmate" : "draw")),
        ...
        ? h(
            ...
            h(
              "color",
              root.trans.noarg(
                end.winner === "white" ? "whiteWinsGame" : "blackWinsGame",
              ),
            ),
          )
  ...
  return h(
    ...
    h("div", {
      attrs: {
        style: `width: ${
          node.ceval
            ? (100 * Math.max(0, node.ceval.depth - minDepth)) /
                (maxDepth - minDepth) +
              "%"
            : 0
        }`,
...
export default function (root: AnalyseCtrl): VNode | undefined {
  ...
  const end = ctrl.currentNode().threefold
    ? { winner: undefined }
    : root.outcome();
  return h(
    "div.practice-box.training-box.sub-box." +
      (comment ? comment.verdict : "no-verdict"),
    [
      h("div.title", root.trans.noarg("practiceWithComputer")),
      h(
        "div.feedback",
        !running
          ? renderOffTrack(root, ctrl)
          : end
          ? renderEnd(root, end)
          : renderRunning(root, ctrl),
      ),
      running
        ? h(
            "div.comment",
            comment
              ? ([
                  ...
                  " ",
                ] as MaybeVNodes).concat(commentBest(comment, root, ctrl))
              : [
                  ctrl.isMyTurn() || end
                    ? ""
                    : h("span.wait", root.trans.noarg("evaluatingYourMove")),
                ],
          )
        : running
        ? h("div.comment")
