...
export default class AnalyseCtrl {
  ...
  constructor(readonly opts: AnalyseOpts, readonly redraw: Redraw) {
    ...
    if (lichess.AnalyseNVUI)
      this.nvui = lichess.AnalyseNVUI(redraw) as NvuiPlugin;
  ...
  private throttleSound = (name: string) =>
  ...
  userMove = (orig: Key, dest: Key, capture?: JustCaptured): void => {
    ...
    const isCapture =
      capture || (piece && piece.role == "pawn" && orig[0] != dest[0]);
  ...
  position(node: Tree.Node): Result<Position, PositionError> {
    ...
    return setupPosition(
      lichessVariantRules(this.data.game.variant.key),
      setup,
    );
  }
