  ...
  if (g.source === "import" && g.importedBy && g.importedBy === userId)
    return h(
      "form.delete",
      {
        ...
        hook: bind("submit", _ =>
          confirm(ctrl.trans.noarg("deleteThisImportedGame")),
        ),
      },
      [
        h(
...
function autoplayButtons(ctrl: AnalyseCtrl): VNode {
  ...
  const speeds = [
    ...baseSpeeds,
    ...(d.game.speed !== "correspondence" && !isEmpty(d.game.moveCentis)
      ? [realtimeSpeed]
      : []),
    ...(d.analysis ? [cplSpeed] : []),
  ];
...
function studyButton(ctrl: AnalyseCtrl) {
  ...
  return h(
    "form",
    {
      ...
      hook: bind("submit", e => {
        const pgnInput = (e.target as HTMLElement).querySelector(
          "input[name=pgn]",
        ) as HTMLInputElement;
    ...
    [
      !ctrl.synthetic
        ? hiddenInput("gameId", ctrl.data.game.id)
        : hiddenInput("pgn", ""),
...
export function view(ctrl: AnalyseCtrl): VNode {
  const d = ctrl.data,
    ...
    canContinue =
      !ctrl.ongoing && !ctrl.embed && d.game.variant.key === "standard",
    ceval = ctrl.getCeval(),
    ...
    h("div.action-menu__tools", [
      ...
      canContinue
        ? h(
            ...
            {
              hook: bind("click", _ =>
                modal($(".continue-with.g_" + d.game.id)),
              ),
              attrs: dataIcon("U"),
            },
                  ...
                  ceval.threads
                    ? (id => {
                        return h("div.setting", [
                          ...
                          h(
                            "div.range_value",
                            `${ceval.threads()} / ${ceval.maxThreads}`,
                          ),
                        ]);
                      })("analyse-threads")
                    : null,
                        ...
                        h("div.setting", [
                          ...
                          h("input#" + id, {
                            ...
                            hook: rangeConfig(
                              () =>
                                Math.floor(
                                  Math.log2(parseInt(ceval.hashSize!())),
                                ),
                          ...
                          h(
                            "div.range_value",
                            formatHashSize(parseInt(ceval.hashSize())),
                          ),
                        ]))("analyse-memory")
                    : null,
                ]
  ...
  return h(
    ...
    tools
      ...
      .concat(cevalConfig)
      .concat(
        ctrl.mainline.length > 4
          ? [h("h2", noarg("replayMode")), autoplayButtons(ctrl)]
          : [],
      )
      .concat([
