...
function view(opts: Opts, coords: Coords): VNode {
  const ctrl = opts.root,
    ...
    onMainline =
      ctrl.tree.pathIsMainline(opts.path) &&
      !ctrl.tree.pathIsForcedVariation(opts.path),
  ...
  return h(
    ...
    [
      ...
      onMainline
        ? null
        : action("S", trans("promoteVariation"), () =>
            ctrl.promote(opts.path, false),
          ),
      onMainline
        ? null
        : action("E", trans("makeMainLine"), () =>
            ctrl.promote(opts.path, true),
          ),
      ...
      .concat([
        onMainline
          ? action("F", trans("forceVariation"), () =>
              ctrl.forceVariation(opts.path, true),
            )
          : null,
      ]),
  );
