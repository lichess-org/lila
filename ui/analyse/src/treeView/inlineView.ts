...
function renderInlined(ctx: Ctx, nodes: Tree.Node[], opts: Opts): LooseVNodes {
  // only 2 branches
...
function renderMoveAndChildrenOf(ctx: Ctx, node: Tree.Node, opts: Opts): LooseVNodes {
  ...
  if (opts.truncate === 0) return [hl('move', { attrs: { p: path } }, '[...]')];
  return [
...
function renderInline(ctx: Ctx, node: Tree.Node, opts: Opts): VNode {
  ...
  if (retro) return hl('interrupt', hl('lines', retro));
  return hl(
    'inline',
...
function renderMoveOf(ctx: Ctx, node: Tree.Node, opts: Opts): VNode {
  ...
  return hl('move', { attrs: { p: path }, class: nodeClasses(ctx, node, path) }, content);
}
