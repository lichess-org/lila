  ...
  return {
    insert: vnode => {
      ...
      const ctxMenuCallback = (e: MouseEvent) => {
        ...
        if (path) contextMenu(e, { path, root: ctrl });
        ctrl.redraw();
        return false;
      };
      if (site.debug) {
      ...
      el.addEventListener('click', (e: MouseEvent) => {
        ...
        console.log('we clicked', ctrl.autoScrollRequested);
        const path = eventPath(e);
    ...
    postpatch: () => {
      console.log('we postpatching', ctrl.autoScrollRequested);
...
export const renderingCtx = (ctrl: AnalyseCtrl): Ctx => ({
  ...
  showComputer: ctrl.showAnalysis() && !ctrl.retro?.isSolving(),
  showGlyphs: (!!ctrl.study && !ctrl.study?.relay) || ctrl.showFishnetAnalysis(),
  showEval: ctrl.showAnalysis(),
