...
import { lishogiCharToRole, lishogiVariantRules } from 'shogiops/compat';
import { squareFile, squareRank, makeSquare, opposite, parseSquare } from 'shogiops/util';
...
function parseMove(san: string, usi: string): ParsedMove {
  usi = pretendItsUsi(usi);
  return {
    role: lishogiCharToRole(san[0])!,
    orig: parseSquare(usi.slice(0, 2))!,
    dest: parseSquare(usi.slice(2, 4))!,
