export default function changeColorHandle(): void {
  ...
  for (const theme of $('body').attr('class').split(' ')) {
    if (theme in dict) {
      document.documentElement.style.setProperty('--cg-coord-color-white', dict[theme][0]);
      document.documentElement.style.setProperty('--cg-coord-color-black', dict[theme][1]);
