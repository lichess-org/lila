...
export default function <Result>(opts: Opts<Result>) {
  const minLength = opts.minLength || 3,
    empty =
      opts.empty ||
      (() => '<div class="complete-list__empty">No results.</div>'),
    ...
    renderSelection = () => {
      ...
      if (selectedIndex !== null)
        $container
          ...
          .eq(selectedIndex)
          .addClass("complete-selected");
    };

  let $container: Cash = $(
      '<div class="complete-list none"></div>',
    ).insertAfter(opts.input),
    ...
    keydown(e: KeyboardEvent) {
      ...
      if (e.code == "Enter") {
        ...
        const result =
          selectedResult() ||
          (renderedResults[0] &&
          opts.populate(renderedResults[0]) == opts.input.value
            ? renderedResults[0]
            : undefined);
        if (result) {
