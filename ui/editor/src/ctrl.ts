...
export default class EditorCtrl {
  ...
  initialFen: string;
  pockets: co.Material | undefined;
  turn: Color;
  unmovedRooks: co.SquareSet | undefined;
  castlingToggles: CastlingToggles<boolean>;
  epSquare: co.Square | undefined;
  remainingChecks: co.RemainingChecks | undefined;
  rules: co.Rules;
  halfmoves: number;
  fullmoves: number;
  ...
  constructor(cfg: Editor.Config, redraw: Redraw) {
    ...
    this.rules = this.cfg.embed
      ? 'chess'
      : co.compat.lichessRules((params.get('variant') || 'standard') as VariantKey);
    this.initialFen = (cfg.fen || params.get('fen') || co.fen.INITIAL_FEN).replace(/_/g, ' ');
  ...
  private getSetup() {
    ...
    const board = co.fen.parseFen(boardFen).unwrap(
      setup => setup.board,
      _ => co.Board.empty(),
    );
    return {
      ...
      turn: this.turn,
      unmovedRooks: this.unmovedRooks || co.fen.parseCastlingFen(board, this.castlingToggleFen()).unwrap(),
      epSquare: this.epSquare,
      remainingChecks: this.remainingChecks,
      halfmoves: this.halfmoves,
      fullmoves: this.fullmoves,
    } as unknown as co.Setup;
  }

  getFen(): string {
    return co.fen.makeFen(this.getSetup());
  }

  private getLegalFen(): string | undefined {
    return co.variant.setupPosition(this.rules, this.getSetup()).unwrap(
      pos => co.fen.makeFen(pos.toSetup()),
      _ => undefined,
    );
  }
  ...
  makeAnalysisUrl(legalFen: string, orientation: Color = 'white'): string {
    const variant = this.rules === 'chess' ? '' : co.compat.lichessVariant(this.rules) + '/';
    return `/analysis/${variant}${urlFen(legalFen)}?color=${orientation}`;
  }

  makeEditorUrl(fen: string, orientation: Color = 'white'): string {
    if (fen === co.fen.INITIAL_FEN && this.rules === 'chess' && orientation === 'white')
      return this.cfg.baseUrl;
    const variant = this.rules === 'chess' ? '' : '?variant=' + co.compat.lichessVariant(this.rules);
  ...
  startPosition = () => this.setFen(co.fen.makeFen(co.variant.defaultPosition(this.rules).toSetup()));

  clearBoard = () => this.setFen(co.fen.EMPTY_FEN);
  ...
  setFen(fen: string): boolean {
    return co.fen.parseFen(fen).unwrap(
      (setup: any) => {
        ...
        const castles = co.Castles.fromSetup(setup);
  ...
  setRules(rules: co.Rules): void {
    this.rules = rules;
    if (rules != 'crazyhouse') this.pockets = undefined;
    else if (!this.pockets) this.pockets = co.Material.empty();
    if (rules != '3check') this.remainingChecks = undefined;
    else if (!this.remainingChecks) this.remainingChecks = co.RemainingChecks.default();
    this.onChange();
  }
