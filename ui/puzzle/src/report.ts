...
export default class Report {
  ...
  checkForMultipleSolutions(ev: ClientEval, ctrl: PuzzleCtrl, threatMode: boolean): void {
    ...
    if (
      ...
      pieceCount(ev.fen) <= 7 ||
      !ctrl.ceval.engines.current()?.capabilities?.includes('puzzleReport') ||
    ...
    if (
      ...
      if (this.evalsWithMultipleSolutions === 2) {
        ...
        const reason = `(v${version}, ${engineName}) after move ${plyToTurn(node.ply)}. ${
          node.san
        }, at depth ${ev.depth}, multiple solutions:\n\n${ev.pvs
          .map(pv => `${pvEvalToStr(pv)}: ${pv.moves.join(' ')}`)
          .join('\n\n')}`;
