  ...
  if (opts.isMainline) {
    ...
    return [
      ...
      ...(isWhite && mainChildren
        ? [renderIndex(main.ply, false), emptyMove()]
        : []),
      ...mainChildren,
    ];
  }
  return cs[1]
    ? [renderLines(ctx, cs, opts)]
...
function renderVariationMoveOf(
  ctx: Ctx,
  node: Tree.Node,
  opts: RenderOpts,
): VNode {
...
function eventPath(e: Event): Tree.Path | null {
  ...
  return (
    ...
    (target.parentNode as HTMLElement).getAttribute("p")
  );
}
...
export function render(ctrl: Controller): VNode {
  ...
  return h(
    ...
    [
      ...(root.ply % 2 === 1
        ? [renderIndex(root.ply, false), emptyMove()]
        : []),
