...
function fitLane(lane, tour2) {
  return !lane.some(function (tour1) {
    return !(
      tour1.finishesAt <= tour2.startsAt || tour2.finishesAt <= tour1.startsAt
    );
  });
}
      ...
      h("span.body", [
        ...
        h("span.infos", [
          h("span.text", [
            ...
            tour.rated
              ? ctrl.trans("ratedTournament")
              : ctrl.trans("casualTournament"),
          ]),
          tour.nbPlayers
            ? h(
                "span.nb-players",
...
function renderTimeline() {
  ...
  time.setSeconds(0);
  time.setMinutes(
    Math.floor(time.getMinutes() / minutesBetween) * minutesBetween
  );
...
export default function (ctrl) {
  ...
  const tourLanes = splitOverlaping(
    group(systemTours, laneGrouper).concat([userTours])
        ...
        hook: {
          insert: (vnode) => {
            ...
            el.scrollLeft = leftPos(
              bitLater - (el.clientWidth / 2.5 / scale) * 60 * 1000
            );
            ...
            el.addEventListener("click", (e) => {
              if (
                mousedownAt &&
                Math.pow(e.clientX - mousedownAt[0], 2) +
                  Math.pow(e.clientY - mousedownAt[1], 2)
              ) {
                e.preventDefault();
                return false;
              }
