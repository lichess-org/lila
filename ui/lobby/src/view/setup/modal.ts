...
export default function setupModal(ctrl: LobbyController): MaybeVNode {
  ...
  return snabDialog({
    attrs: { dialog: { role: 'dialog', 'aria-labelledBy': 'dialog_label', 'aria-modal': 'true' } },
    class: 'game-setup',
...
const views = {
  hook: (ctrl: LobbyController): MaybeVNodes => [
    h('h2#dialog_label', i18n.site.createAGame),
    h('div.setup-content', [
      variantPicker(ctrl),
  ...
  friend: (ctrl: LobbyController): MaybeVNodes => [
    h('h2#dialog_label', i18n.site.playWithAFriend),
    h('div.setup-content', [
  ...
  ai: (ctrl: LobbyController): MaybeVNodes => [
    h('h2#dialog_label', i18n.site.playWithTheMachine),
    h('div.setup-content', [
      variantPicker(ctrl),
      fenInput(ctrl),
      timePickerAndSliders(ctrl, true),
