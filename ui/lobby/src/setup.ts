...
export default class Setup {
  ...
  constructor(
    readonly makeStorage: (name: string) => LichessStorage,
  ...
  prepareForm = ($modal: Cash) => {
    ...
    const showRating = () => {
      ...
      switch ($variantSelect.val()) {
        ...
        case "3":
          if (timeMode == "1") {
            const time =
              parseFloat($timeInput.val() as string) * 60 +
              parseFloat($incrementInput.val() as string) * 40;
    ...
    if (typ == "hook") {
      ...
      const ajaxSubmit = (color: string) => {
        ...
        const rating =
          parseInt($modal.find(".ratings input").val() as string) || 1500;
        if (form.ratingRange)
        ...
        if (poolMember) {
          ...
          this.root.setTab(
            $timeModeSelect.val() === "1" ? "real_time" : "seeks",
          );
          xhr.text(
    ...
    var validateFen = debounce(() => {
      ...
      if (fen)
        xhr
          .text(xhr.url($fenInput.parent().data("validate-url"), { fen }))
          .then(
            data => {
              ...
              $fenPosition
                .find("a.board_editor")
                .each(function (this: HTMLAnchorElement) {
