...
export default class SetupController {
  ...
  private storeKey = (gameType: GameType) =>
  ...
  private loadPropsFromStore = (forceOptions?: ForceSetupOptions) => {
    ...
    this.variant = this.propWithApplyVariant(
      forceOptions?.variant || storeProps.variant
    );
    this.fen = this.propWithApply(forceOptions?.fen || storeProps.fen);
    this.timeMode = this.propWithApply(
      forceOptions?.timeMode || storeProps.timeMode
    );
    this.timeV = this.propWithApply(
      sliderInitVal(storeProps.time, timeVToTime, 100)!
    );
    ...
    this.daysV = this.propWithApply(
      sliderInitVal(storeProps.days, daysVToDays, 20)!
    );
  ...
  private enforcePropRules = () => {
    ...
    if (this.variant() === "fromPosition")
      this.fen = this.propWithApply(this.fen().replace(/_/g, " "));
  ...
  private onPropChange = () => {
    this.enforcePropRules();
    if (
      this.root.data.ratingMap &&
      this.selectedPerf() &&
      !!this.root.data.ratingMap[this.selectedPerf()].prov
    ) {
  ...
  private propWithApply = <A>(value: A) =>
    propWithEffect(value, this.onPropChange);
  ...
  selectedPerf = (): Perf =>
    getPerf(this.variant(), this.timeMode(), this.time(), this.increment());
  ...
  validTime = (): boolean =>
    this.timeMode() !== "realTime" || this.time() > 0 || this.increment() > 0;
  validAiTime = (): boolean =>
    this.gameType !== "ai" ||
    this.timeMode() !== "realTime" ||
    ...
    this.time() >= 1;
  valid = (): boolean =>
    this.validFen() && this.validTime() && this.validAiTime();
  ...
  submit = async (color: Color | "random") => {
    ...
    if (this.gameType === "hook")
      this.root.setTab(this.timeMode() === "realTime" ? "real_time" : "seeks");
    this.loading = true;
    this.root.redraw();
    ...
    try {
      ...
      this.root.redraw();
      alert(
        "Sorry, we encountered an error while creating your game. Please try again."
      );
      return;
    }
