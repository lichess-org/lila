...
function renderTournament(tour: Tournament, day: Date) {
  let left =
    ((getHours(tour.bounds.start) + getMinutes(tour.bounds.start) / 60) / 24) *
    100;
...
function renderDay(ctrl: Ctrl) {
  return function (day: Date): VNode {
    ...
    const tours = ctrl.data.tournaments.filter(
      t => t.bounds.start < dayEnd && t.bounds.end > day,
    );
    return h("day", [
      h(
        "date",
...
export default function (ctrl) {
  ...
  return h(
    "div#tournament-calendar",
    h("groups", groups.map(renderGroup(ctrl))),
  );
}
