...
export default function (token: string) {
  ...
  const announceAllMoves =
    localStorage.getItem("dgt-speech-announce-all-moves") == "true";
  const verbose = localStorage.getItem("dgt-verbose") == "true";
  const announceMoveFormat = localStorage.getItem(
    "dgt-speech-announce-move-format",
  )
    ? localStorage.getItem("dgt-speech-announce-move-format")
    : "san";
  const speechSynthesisOn =
    localStorage.getItem("dgt-speech-synthesis") == "true";
  ...
  try {
    if (JSON.parse(localStorage.getItem("dgt-speech-keywords")!).K.length > 0) {
      ...
      console.warn(
        "JSON Object for Speech Keywords seems incompelte. Using English default.",
      );
    }
  } catch (error) {
    console.error(
  ...
  var gameConnectionMap = new Map<
    string,
    { connected: boolean; lastEvent: number }
  >(); //A collection of key values to store the network status of a game
        ...
        return new Promise<void>(resolve => {
          ...
          for (let i = 0; i < arguments.length; i++) {
            ...
            if (arg == "*" || arg == ":") {
              ...
              output +=
                '</br><span class="log-' + typeof arg + " log-" + name + '">';
          ...
          if (autoScroll) {
            isScrolledToBottom =
              eleOverflowLocator.scrollHeight -
                eleOverflowLocator.clientHeight <=
              eleOverflowLocator.scrollTop + 1;
          }
          eleLocator.innerHTML =
            eleLocator.innerHTML.slice(maxLogBytes) + output;
          if (isScrolledToBottom) {
            eleOverflowLocator.scrollTop =
 ...
 */
  async function connectToEventStream() {
    ...
    while (true) {
      ...
      if (verbose && value!.length > 1)
        console.log(
          "connectToEventStream - Chunk received",
        ...
        if (jsonArray[i].length > 2) {
          try {
            ...
            if (data.type == "gameStart") {
              if (verbose)
                console.log(
                  "connectToEventStream - gameStart event arrived. GameId: " +
                    data.game.id,
                );
              try {
  ...
  async function connectToGameStream(gameId: string) {
    ...
    if (verbose)
      console.log(
        "connectToGameStream - About to call /api/board/game/stream/" + gameId,
      );
          ...
          try {
            ...
            if (data.type == "gameFull") {
              ...
              if (gameId == currentGameId) {
                ...
                if (verbose)
                  console.log(
                    "connectToGameStream - State received was not for current game.",
                  );
              }
  ...
  async function lichessConnectionLoop() {
    ...
    for (let attempts = 0; attempts < 20; attempts++) {
      ...
      do {
        ...
        for (let [gameId, networkState] of gameConnectionMap) {
          if (
            ...
            gameStateMap.get(gameId).status == "started"
          ) {
            //Game is not conencted and has not finished, reconnect
            if (verbose)
              console.log(
                `Started game is disconnected. Attempting reconnection for gameId: ${gameId}`,
              );
      ...
      console.warn(
        "No connection to event stream. Attempting re-connection. Attempt: " +
          attempts,
      );
    }
    console.error(
    ...
    */
    if (playableGames.length == 0) {
      ...
      for (let i = 0; i < playableGames.length; i++) {
        ...
        let tmpFEN = fen.makeBoardFen(
          gameChessBoardMap.get(playableGames[i].gameId)!.board,
        );
        if (verbose)
      ...
      if (index == -1) {
        ...
        if (
          ...
          console.warn(
            "Fix position and reload or start a new game. Automatically retrying in 5 seconds...",
          );
          await sleep(5000);
          chooseCurrentGame();
        }
        ...
        if (currentGameId != playableGames[Number(index)].gameId) {
          ...
          if (verbose)
            console.log(
              "chooseCurrentGame - Position matched to gameId: " +
                playableGames[Number(index)].gameId,
            );
  ...
  function initializeChessBoard(
    gameId: string,
    data: { initialFen: string; state: { moves: string } },
  ) {
    try {
  ...
  function updateChessBoard(
    gameId: string,
    currentState: { moves: string },
    newState: { moves: string },
  ) {
    try {
      ...
      if (chess) {
        ...
        if (!currentState.moves) {
          ...
          pendingMoves = newState.moves.substring(
  ...
  function playableGamesArray(): Array<{
    ...
    for (var i = 0; i < keys.length; i++) {
      if (
        gameConnectionMap.get(keys[i])?.connected &&
        gameStateMap.get(keys[i])?.status == "started"
      ) {
        ...
        playableGames.push({
          ...
          "vs rating":
            gameInfo.black.id == me.id
              ? gameInfo.white.rating
              : gameInfo.black.rating,
          "game rating":
            gameInfo.variant.short +
            " " +
            (gameInfo.rated ? "rated" : "unrated"),
          "Timer":
            gameInfo.speed +
            " " +
            (gameInfo.clock !== null
              ? String(gameInfo.clock.initial / 60000) +
                "'+" +
                String(gameInfo.clock.increment / 1000) +
                "''"
              : "∞"),
          "Last Move":
            lastMove.player + " " + lastMove.move + " by " + lastMove.by,
        });
      }
    }
    ...
    if (gameStateMap.has(gameId) && gameInfoMap.has(gameId)) {
      ...
      */
      var innerTable =
        ...
        `<tr><td>White</td><td>${
          gameInfo.white.title !== null ? gameInfo.white.title : "@"
        }</td><td>${gameInfo.white.name}</td><td>${
          gameInfo.white.rating
        }</td><td>${formattedTimer(gameState.wtime)}</td><td>${
        ...
        `<tr><td>Black</td><td>${
          gameInfo.black.title !== null ? gameInfo.black.title : "@"
        }</td><td>${gameInfo.black.name}</td><td>${
          gameInfo.black.rating
        }</td><td>${formattedTimer(gameState.btime)}</td><td>${
   ...
   */
  function getLastUCIMove(
    gameId: string,
  ): { player: string; move: string; by: string } {
    if (gameStateMap.has(gameId) && gameInfoMap.has(gameId)) {
      var gameInfo = gameInfoMap.get(gameId);
      var gameState = gameStateMap.get(gameId);
      ...
      if (String(gameState.moves).length > 1) {
        ...
        if (verbose)
          console.log(
            `getLastUCIMove - ${moves.length} moves detected. Last one: ${
        ...
        if (moves.length % 2 == 0)
          return {
            ...
            by: gameInfo.black.id,
          };
        else
          return {
            player: "white",
            move: moves[moves.length - 1],
            by: gameInfo.white.id,
          };
      }
    }
  ...
  function announcePlay(lastMove: {
    ...
    if (
      announceMoveFormat &&
      announceMoveFormat.toLowerCase() == "san" &&
      lastSanMove
    ) {
    ...
    if (lastMove.player == "white") {
      console.log(
        '<span class="dgt-white-move">' + moveText + " by White" + "</span>",
      );
    } else {
      console.log(
        '<span class="dgt-black-move">' + moveText + " by Black" + "</span>",
      );
    }
    //TODO
  ...
  async function connectToLiveChess() {
    ...
    //Open the WebSocket
    liveChessConnection = new WebSocket(
      liveChessURL ? liveChessURL : "ws://localhost:1982/api/v1.0",
    );
    ...
    liveChessConnection.onmessage = async e => {
      ...
      if (message.response == "call" && message.id == "1") {
        ...
        if (verbose)
          console.info(
            "Websocket onmessage[call]: board serial number updated to: " +
              currentSerialnr,
          );
        if (verbose)
          console.info(
        ...
        if (boards[0].state != "ACTIVE" && boards[0].state != "INACTIVE")
          ...
          console.error(
            `Board with serial ${currentSerialnr} is not properly connected. Please fix`,
          );
        ...
        if (
          ...
          if (currentGameId != DGTgameId) {
            ...
            if (verbose)
              console.info(
                "There is a game in progress, calling liveChessBoardSetUp...",
              );
            sendBoardToLiveChess(gameChessBoardMap.get(currentGameId)!);
          }
        }
        ...
        if (message.param.san.length == 0) {
          ...
          JSON.stringify(lastLegalParam.san) ==
            JSON.stringify(message.param.san)
        ) {
          //Prevent duplicates since LiveChess may send the same move twice
          //It looks like a duplicate, so just ignore it
          if (verbose)
            console.info(
              "onmessage - Duplicate position and san move received and will be ignored",
            );
        } else {
          //A move was received
          ...
          if (lastLegalParam !== undefined)
            movesToProcess =
              message.param.san.length - lastLegalParam.san.length;
          //Check border case in which DGT Board LiveChess detects the wrong move while pieces are still on the air
          if (movesToProcess > 1) {
            if (verbose)
              console.warn(
                "onmessage - Multiple moves received on single message - movesToProcess: " +
                  movesToProcess,
              );
            if (localBoard.turn == currentGameColor) {
              ...
              if (
                JSON.stringify(lastLegalParam.san) !=
                JSON.stringify(quarentinedlastLegalParam.san)
              ) {
          ...
          for (let i = movesToProcess; i > 0; i--) {
            ...
            SANMove = String(
              message.param.san[message.param.san.length - i],
            ).trim();
            ...
            if (moveObject && localBoard.isLegal(moveObject)) {
              ...
              if (lastMove.move == SANMove) {
                ...
                if (verbose)
                  console.warn(
                    "onmessage - Move received is the same as the last moved played: " +
                      SANMove,
                  );
                ...
                if (verbose)
                  console.warn(
                    "onmessage - Caslting may be duplicated as the last moved played: " +
                      SANMove,
                  );
              } else {
  ...
  async function DGTliveChessConnectionLoop() {
    ...
    for (let attempts = 0; attempts < 20; attempts++) {
      ...
      if (!isLiveChessConnected) {
        console.warn(
          "No connection to DGT Live Chess. Attempting re-connection. Attempt: " +
            attempts,
        );
        connectToLiveChess();
      } else {
          ...
          try {
            if (response.status == 200 || response.status == 201) {
              ...
              response.json().then(errorJson => {
                console.error(
                  "sendMove - Failed to send move. " + errorJson.error,
                );
              });
            }
          } catch (error) {
  ...
  function replaceKeywords(sanMove) {
    ...
    for (let i = 0; i < keywordsBase.length; i++) {
      try {
        ...
        console.error(
          `raplaceKeywords - Error replacing keyword. ${keywordsBase[i]} . ${
            Error(error).message
          }`,
        );
      }
  ...
  function compareMoves(lastMove: string, moveObject: NormalMove): boolean {
    try {
      ...
      if (verbose)
        console.log(
          "Moves look diffrent. Check if this is a castling mismatch.",
        );
  ...
  */
  ...
  function start() {
    ...
    console.log(
      "    `\\_..==`` /                 Lichess.org - DGT Electronic Board Connector   ",
    );
    console.log(
