  ...
  const submit: Submit = (v: string, submitOpts: SubmitOpts) => {
    ...
    if (!submitOpts.submitCommand) {
      if (move) {
        if (isDrop(move))
          shapes.push({
            orig: { color: pos!.turn, role: move.role },
        ...
        if (sqs.length === 1) {
          ...
          if (
            ...
            shapes.push({
              orig: { color: pos.turn, role },
              dest: { color: pos.turn, role },
              brush,
            });
        }
      }
      ...
      if (move) {
        ...
        if (wrong && !opts.input.classList.contains('wrong')) window.lishogi.sound.play('error');
