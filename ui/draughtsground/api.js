...
function start(state, redrawAll) {
    ...
    ;
    return {
        set: function (config, noCaptSequences) {
            if (noCaptSequences === void 0) { noCaptSequences = false; }
            if (config.orientation && config.orientation !== state.orientation)
                toggleOrientation();
            if (config.fen) {
                anim_1.anim(function (state) { return config_1.configure(state, config); }, state, false, noCaptSequences);
                if (state.selected && !state.pieces[state.selected])
                    state.selected = undefined;
            }
            else
                anim_1.render(function (state) { return config_1.configure(state, config); }, state);
        },
        state: state,
        ...
        playPremove: function () {
            if (state.premovable.current) {
                var dest = state.premovable.current ? state.premovable.current[1] : "00";
