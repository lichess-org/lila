...
export default async function (data: any, { singlePerfName }: Opts) {
  ...
  $el.each(function (this: HTMLElement) {
    ...
    addEventListener("resize", () => Plotly.Plots.resize(this));
    ...
    this.textContent = '';
    Plotly.newPlot(this, usedData.map((data, i) => ({name: data.name, ...smoothDates(data.points), line: styles[i]})), {
      showlegend: false,
      hovermode: 'x unified',
