...
export default function (data: any, trans: Trans, hunter: boolean) {
  ...
  lichess.loadModule('chart.common').then(function () {
    window.LichessChartCommon('highchart').then(function () {
      window.LichessChartMovetime.render = () => {
            ...
            events: {
              click: (event: any) => {
                if (event.point) {
                  ...
                  const p =
                    chartElm.highcharts.series[(showTotal ? 4 : 0) + (((tree[x] ? tree[x].ply : undefined) || x) % 2)]
                      .data[x >> 1];
                  if (p) p.select(true);
