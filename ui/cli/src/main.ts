...
function commandHelp(aliases: string, args: string, desc: string) {
  return '<div class="command"><div>' +
    aliases.split(' ').map(a => `<p>/${a} ${window.lidraughts.escapeHtml(args)}</p>`).join('') +
    `</div> ${desc}</div>`;
}

function help() {
  window.lidraughts.loadCssPath('clinput.help')
  $.modal(
    '<h3>Commands</h3>' +
...
function getDasher(cb: (dasher: any) => void) {
  window.lidraughts.loadScript(window.lidraughts.compiledScript('dasher')).done(function() {
    window['LidraughtsDasher'].default(document.createElement('div'), {
