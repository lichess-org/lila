...
export function renderClock(
  ...
  return h(
    ...
    clock.opts.nvui
      ...
      : [
          clock.showBar && game.bothPlayersHavePlayed(ctrl.data)
...
function showBar(ctrl: RoundController, color: Color) {
  ...
  const update = (el: HTMLElement) => {
    if (el.animate !== undefined) {
      ...
      el.style.transform =
        "scale(" + clock.timeRatio(clock.millisOf(color)) + ",1)";
    }
  };
  return h("div.bar", {
...
function showBerserk(ctrl: RoundController, color: Color): boolean {
  return (
    !!ctrl.goneBerserk[color] &&
    ctrl.data.game.turns <= 1 &&
    game.playable(ctrl.data)
  );
}

function renderBerserk(
  ctrl: RoundController,
  color: Color,
  position: Position,
) {
  return showBerserk(ctrl, color)
    ? h("div.berserked." + position, justIcon("`"))
    : null;
}
