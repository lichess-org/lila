...
function analysisButton(ctrl: RoundController): VNode | null {
  ...
  return game.replayable(d) ? h('a.button.button-empty', {
    attrs: { href: url },
...
export function followUp(ctrl: RoundController): VNode {
  ...
  return h('div.follow-up', [
    ...
    newable ? h('a.button.button-empty', {
...
const onSuggestionHook: Hooks = {
  insert(vnode) {
    window.lidraughts.pubsub.emit('round.suggestion')((vnode.elm as HTMLElement).textContent);
  }
};
