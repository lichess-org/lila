      ...
      "button.fbt.rematch.white",
      {
        class: {
          ...
          disabled:
            ...
            !(
              d.opponent.onGame ||
              (!d.clock && d.player.user && d.opponent.user)
            ),
        },
        attrs: {
          title: them
            ? noarg("yourOpponentWantsToPlayANewGameWithYou")
            : me
        ...
        hook: util.bind(
          ...
          e => {
            const d = ctrl.data;
            if (d.game.rematch)
              location.href = gameRoute(d.game.rematch, d.opponent.color);
              ...
              !(e.target as HTMLElement).classList.contains("disabled")
            )
  ...
  return gone === true
    ? h("div.suggestion", [
        ...
        h(
          "button.button",
          {
            hook: util.bind("click", () =>
              ctrl.socket.sendLoading("resign-force"),
            ),
          },
        ...
        h(
          ...
          {
            hook: util.bind("click", () =>
              ctrl.socket.sendLoading("draw-force"),
            ),
          },
  ...
  return ctrl.data.game.threefold
    ? h("div.suggestion", [
        ...
        h(
          ...
          {
            hook: util.bind("click", () =>
              ctrl.socket.sendLoading("draw-claim"),
            ),
          },
...
export function cancelDrawOffer(ctrl: RoundController) {
  return ctrl.data.player.offeringDraw
    ? h("div.pending", [h("p", ctrl.noarg("drawOfferSent"))])
    : null;
}
...
export function answerOpponentDrawOffer(ctrl: RoundController) {
  return ctrl.data.opponent.offeringDraw
    ? h("div.negotiation.draw", [
        ...
        acceptButton(ctrl, "draw-yes", () =>
          ctrl.socket.sendLoading("draw-yes"),
        ),
  ...
  return ctrl.data.player.proposingTakeback
    ? h("div.pending", [
        ...
        h(
          ...
          {
            hook: util.bind("click", () =>
              ctrl.socket.sendLoading("takeback-no"),
            ),
          },
          ctrl.noarg("cancel"),
        ),
      ...
      ? h(
          "a.fbt",
          {
            attrs: {
              href:
                d.game.source === "pool"
                  ? poolUrl(d.clock!, d.opponent.user)
                  : "/?hook_like=" + d.game.id,
            },
          },
...
const onSuggestionHook: Hooks = util.onInsert(el =>
  lichess.pubsub.emit("round.suggestion", el.textContent),
);
