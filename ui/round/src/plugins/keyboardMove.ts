...
lichess.keyboardMove = function (opts: Opts) {
  ...
  const submit: Submit = function (v: string, submitOpts: SubmitOpts) {
    ...
    if (v == "resign") {
      ...
      if (!foundUci) return;
      opts.ctrl.promote(
        foundUci.slice(0, 2) as Key,
        foundUci.slice(2) as Key,
      ...
      const wrong =
        v.length && legalSans && !sanCandidates(v, legalSans).length;
  ...
  return (fen: string, dests: Dests | undefined, yourMove: boolean) => {
    legalSans =
      dests && dests.size > 0 ? sanWriter(fen, destsToUcis(dests)) : null;
