...
import { throttlePromise, throttlePromiseDelay } from 'common/throttle';
...
export default class RacerCtrl {
  ...
  join = throttlePromise(1000, () => {
    if (!this.isPlayer()) this.socketSend('racerJoin');
  });

  start = throttlePromise(1000, () => {
  ...
  private setZen = throttlePromiseDelay(1000, zen =>
    xhr.text('/pref/zen', {
      method: 'post',
