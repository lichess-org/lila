const searchParams: URLSearchParams = new URL(self.location.href).searchParams;
const assetBase: string = new URL(searchParams.get('asset-url')!, self.location.href).href;
...
self.addEventListener('push', event => {
  ...
  return event.waitUntil(
    self.registration.showNotification(data.title, {
      badge: assetUrl('logo/lishogi-mono-128.png'),
      icon: assetUrl('logo/lishogi-favicon-192.png'),
      body: data.body,
      tag: data.tag,
      data: data.payload,
