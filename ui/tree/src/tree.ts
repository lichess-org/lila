...
export function build(root: Tree.Node): TreeWrapper {
    ...
    /*function expandSan(node: Tree.Node) {
        if (node.san && node.uci && node.uci.length > 4) {
            node.expandedSan = "";
            for (let i = 0; i + 2 <= node.uci.length; i += 2) {
                if (node.expandedSan.length !== 0) node.expandedSan += "x"
                if (node.uci.substr(i, 1) === "0")
                    node.expandedSan += node.uci.substr(i + 1, 1)
                else
                    node.expandedSan += node.uci.substr(i, 2)
            }
        }
    }

    function collapsedSan(node: Tree.Node) {
        if (node.uci && node.uci.length > 4) {
            const src = node.uci.substr(0, 2);
            const dst = node.uci.substr(node.uci.length - 2, 2);
            return (src.substr(0, 1) === "0" ? src.substr(1, 1) : src) + "x" + (dst.substr(0, 1) === "0" ? dst.substr(1, 1) : dst);
        } else return node.san ? node.san : "";
    }*/
    ...
    function addNode(newNode: Tree.Node, path: Tree.Path, puzzleEditor: Boolean = false): Tree.Path | undefined {
        ...
        if (curNode && countGhosts(curNode.fen) > 0) {
