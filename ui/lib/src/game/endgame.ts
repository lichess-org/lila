...
export function endgameHighlights(
  ...
  if (winner) {
    ...
    const endgameIcons: Record<string, string> = {
      ...
      timeout: '<path fill="#fff" d="M68 10h8v4h-8zM78 16l4 4-10 10-4-4zM66 17a11 11 0 1 0 16 16l-4-4a5 5 0 1 1-8-8z"/>',
      draw: '<path fill="#fff" d="M62 15h27v6H62zM62 27h27v6H62z"/>',
    };

    const endgameSvgFor = (kind: keyof typeof endgameIcons): string =>
      endgameSvg(
        kind === 'win' ? '#22ac38' : kind === 'draw' ? '#c8a829' : '#df5353',
        endgameIcons[kind],
      );
