...
object ApplicationBuild extends Build {
  ...
  lazy val root = Project("lila", file(".")) enablePlugins PlayScala settings (
    ...
    libraryDependencies ++= Seq(
      ...
      csv, jgit, laika, elastic4s, findbugs, RM,
  ...
  lazy val pool = project("pool", Seq(
    common, hub, socket, chess, game, round, setup, security, chat, memo, user, rating)).settings(
    libraryDependencies ++= provided(play.api, RM, PRM)
  )
  ...
  lazy val wiki = project("wiki", Seq(common, db)).settings(
    libraryDependencies ++= provided(
      play.api, RM, PRM, jgit, laika, guava)
  )
