...
lazy val puzzle = module("puzzle", Seq(
  ...
  libraryDependencies ++= Seq(play.api) ++ reactivemongo.bundle
)

lazy val quote = module("quote", Seq()).settings(
  libraryDependencies ++= Seq(play.api)
)
...
lazy val blog = module("blog", Seq(common, memo, timeline)).settings(
  libraryDependencies ++= Seq(play.api, prismic) ++ reactivemongo.bundle
)
...
lazy val common = module("common", Seq()).settings(
  libraryDependencies ++= Seq(play.api, kamon.core, scalatags, jodaForms, scaffeine) ++ reactivemongo.bundle
)
...
lazy val history = module("history", Seq(common, db, memo, game, user)).settings(
  libraryDependencies ++= Seq(play.api, scalatags) ++ reactivemongo.bundle
)

lazy val db = module("db", Seq(common)).settings(
  libraryDependencies ++= Seq(play.api, hasher, scrimage) ++ reactivemongo.bundle
)
...
lazy val room = module("room", Seq(common, socket, chat)).settings(
  libraryDependencies ++= Seq(play.api, lettuce) ++ reactivemongo.bundle
)
...
lazy val user = module("user", Seq(common, memo, db, hub, rating, socket)).settings(
  libraryDependencies ++= Seq(play.api, hasher) ++ reactivemongo.bundle
)
...
lazy val round = module("round", Seq(
  ...
  libraryDependencies ++= Seq(play.api, scalatags, hasher, kamon.core, lettuce) ++ reactivemongo.bundle
)
...
lazy val tournament = module("tournament", Seq(
  ...
  libraryDependencies ++= Seq(play.api, scalatags, lettuce) ++ reactivemongo.bundle
)
...
lazy val security = module("security", Seq(common, hub, db, user, i18n, slack, oauth)).settings(
  libraryDependencies ++= Seq(play.api, scalatags, maxmind, hasher) ++ reactivemongo.bundle
)
...
lazy val relay = module("relay", Seq(common, study)).settings(
  libraryDependencies ++= Seq(scalaUri) ++ Seq(play.api, markdown) ++ reactivemongo.bundle
)
...
lazy val i18n = module("i18n", Seq(common, db, user, hub)).settings(
  ...
  libraryDependencies ++= Seq(play.api, scalatags)
)
...
lazy val socket = module("socket", Seq(common, hub, memo, tree)).settings(
  libraryDependencies ++= Seq(play.api, lettuce)
)
