...
final class StreamerBits(helpers: Helpers)(picfitApi: lila.core.misc.PicfitApi):
  ...
  object thumbnail:
    ...
    def url(s: Streamer) =
      s.picture match
        case Some(image) => picfitApi.thumbnailUrl(image, Streamer.imageSize, Streamer.imageSize)
