      ...
      userApi withPerfs name.id flatMap {
        ...
        .soFu: u =>
          for
            ...
            percentileLow = perfStat.lowest.flatMap{r => calc_percentile(distribution, r.int)}
            percentileHigh = perfStat.highest.flatMap{r => calc_percentile(distribution, r.int)}
