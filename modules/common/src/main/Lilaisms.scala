...
trait Lilaisms
  ...
  @inline implicit def toPimpedFutureOption[A](f: Fu[Option[A]]): PimpedFutureOption[A] = new PimpedFutureOption(f)
  @inline implicit def toPimpedIterableFuture[A, M[X] <: IterableOnce[X]](t: M[Fu[A]]): PimpedIterableFuture[A,M] =
  ...
  @inline implicit def toPimpedFiniteDuration(d: FiniteDuration): PimpedFiniteDuration = new PimpedFiniteDuration(d)
