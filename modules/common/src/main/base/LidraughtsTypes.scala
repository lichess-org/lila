...
trait LidraughtsTypes extends ValidTypes {
  ...
  def fufail[X](s: String): Fu[X] = fufail(LidraughtsException(s))
  def fufail[X](f: Failures): Fu[X] = fufail(LidraughtsException(f))
  val funit = fuccess(())
...
object LidraughtsTypes extends LidraughtsTypes