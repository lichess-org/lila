...
class BinaryPgnTest extends Specification with WithColl {

  val id = "00000000"

  private def saveAndGet(repo: PgnRepo, moves: List[String]): List[String] = 
    (repo.save(id, moves) >> repo.get(id)).await(Duration(1, SECONDS))

  "binary pgn" should {
    "be isomorphic" in {
      val moves = List("e4", "d5")
      withColl { c ⇒
        saveAndGet(new PgnRepo { def coll = c }, moves) must_== moves
      }
    }
