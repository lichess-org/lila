  ...
  "binary move times" should {
    ...
    "buckets - long game" in {
      val times = Vector(
        ...
        72, 75, 78, 81, 84, 87, 90, 93, 96, 99, 102, 105, 108, 199, 333, 567, 666, 2000,
      ).map(Centis(10) * _)
      val rounded = BinaryFormat.moveTime.read(BinaryFormat.moveTime.write(times), times.size)
      val expected = Vector(
        1, 1, 5, 10, 10, 15, 20, 20, 20, 30, 30, 30, 40, 40, 40, 40, 50, 50, 50, 60, 60, 60, 60, 60,
        80, 80, 80, 80, 80, 80, 80, 100, 100, 100, 100, 100, 100, 200, 300, 600, 600, 600,
      ).map(Centis(10) * _)
