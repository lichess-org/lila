...
object Namer {
  ...
  def playerTextBlocking(player: Player, withRating: Boolean = false)(implicit lightUser: LightUser.GetterSync): String =
    playerTextUser(player, player.userId flatMap lightUser, withRating)

  def playerText(player: Player, withRating: Boolean = false)(implicit lightUser: LightUser.Getter): Fu[String] =
  ...
  def gameVsTextBlocking(game: Game, withRatings: Boolean = false)(implicit lightUser: LightUser.GetterSync): String =
  ...
  def gameVsText(game: Game, withRatings: Boolean = false)(implicit lightUser: LightUser.Getter): Fu[String] =
    game.whitePlayer.userId.??(lightUser) zip
      game.whitePlayer.userId.??(lightUser) dmap {
        case (wu, bu) =>
