...
final class AdminUi(helpers: Helpers, bits: TeamUi)(using netDomain: NetDomain):
  ...
  def leaders(
    ...
    Page(s"${t.name} • ${trans.team.teamLeaders.txt()}", _.js(EsmInit("mod.teamAdmin")).cssTag("tagify")):
  ...
  def kick(t: Team, form: Form[String], blocklistForm: Form[String])(using PageContext) = bits.teamPage:
    Page(s"${t.name} • ${trans.team.kickSomeone.txt()}", _.js(EsmInit("mod.teamAdmin")).cssTag("tagify")):
