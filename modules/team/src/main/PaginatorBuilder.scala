...
final private[team] class PaginatorBuilder(
  ...
  def declinedRequests(team: Team, page: Int, userQuery: Option[String] = None): Fu[Paginator[RequestWithUser]] =
    Paginator(
      adapter = DeclinedRequestAdapter(team, userQuery),
      page,
      maxRequestsPerPage
    )
  final private class DeclinedRequestAdapter(team: Team, userQuery: Option[String] = None) extends AdapterLike[RequestWithUser]:
