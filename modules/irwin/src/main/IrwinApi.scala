...
import lidraughts.game.{ Game, Pov, GameRepo, Query }
import lidraughts.report.{ Report, Mod, Suspect, Reporter, SuspectId, ModId }
import lidraughts.tournament.{ Tournament, TournamentTop }
...
final class IrwinApi(
  ...
  object reports {
    ...
    def insert(report: IrwinReport) = (mode() != "none") ?? {
      for {
        ...
        lidraughts.mon.mod.irwin.ownerReport(report.owner)()
      }
    }
    ...
    private def markOrReport(report: IrwinReport): Funit =
      if (report.activation >= markThreshold && mode() == "mark")
        ...
        _ <- reportApi.create(Report.Candidate(
          ...
          suspect = suspect,
          reason = lidraughts.report.Reason.Cheat,
          text = s"${report.activation}% over ${report.games.size} games"
        ))
      } yield lidraughts.mon.mod.irwin.report()
      else funit
  }
  ...
  object requests {
    ...
    private[irwin] def fromTournamentLeaders(leaders: Map[Tournament, TournamentTop]): Funit =
      lidraughts.common.Future.applySequentially(leaders.toList) {
        case (tour, top) =>
          UserRepo byIds top.value.zipWithIndex
            .filter(_._2 <= tour.nbPlayers * 2 / 100)
            .map(_._1.userId)
            .take(20) flatMap { users =>
              lidraughts.common.Future.applySequentially(users) { user =>
    ...
    private[irwin] def fromLeaderboard(leaders: List[User]): Funit =
      lidraughts.common.Future.applySequentially(leaders) { user =>
  ...
  object notification {
    ...
    private[IrwinApi] def apply(report: IrwinReport): Funit =
      subs.get(report.suspectId) ?? { modId =>
        ...
        import lidraughts.notify.{ Notification, IrwinDone }
