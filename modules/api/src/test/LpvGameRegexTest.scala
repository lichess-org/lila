...
class LpvGameRegexTest extends munit.FunSuite {
  ...
  test("forum links match basic") {
    assert(re.forumGamePgnsRe.matches("https://boo.org:8080/1234abcd"))
  }

  test("forum links match color") {
    assert(re.forumGamePgnsRe.matches("https://boo.org:8080/abcd1234/white"))
  }

  test("forum links match relative") {
    assert(re.forumGamePgnsRe.matches("/12345678/black#123"))
  }

  test("forum links match no scheme") {
    assert(re.forumGamePgnsRe.matches("boo.org:8080/abcdefgh#123"))
  }

  test("forum links match full game id") {
    assert(re.forumGamePgnsRe.matches("boo.org:8080/12345678abcd#12"))
  }

  test("forum links fail wrong host") {
    assert(!re.forumGamePgnsRe.matches("boo.org:1234/zyxwvuts"))
  }

  test("forum links fail too long") {
    assert(!re.forumGamePgnsRe.matches("/abcdefghijkl1234"))
  }

  test("forum links fail bad ply") {
    assert(!re.forumGamePgnsRe.matches("/abcdefgh#12noes"))
  }

  test("forum links fail bad color") {
    assert(!re.forumGamePgnsRe.matches("https://boo.org:8080/12345678/red#22"))
  }

  test("forum links fail tournaments") {
    assert(!re.forumGamePgnsRe.matches("https://boo.org:8080/tournament/ABCD1234"))
  }

  test("forum links fail studies") {
    assert(!re.forumGamePgnsRe.matches("https://boo.org:8080/study/abcd1234/bcde2345"))
  }

  test("forum links fail site header") {
    assert(!re.forumGamePgnsRe.matches("[Site=\"https://boo.org:8080/abcd1234\"]"))
  }

  test("forum links fail mid-line") {
    assert(!re.forumGamePgnsRe.matches("once upon a time /1234abcd/white#4"))
  }

  test("forum links fail usernames") {
    assert(!re.forumGamePgnsRe.matches("/@/thibault"))
  }

  test("forum links fail full id AND color") {
    assert(!re.forumGamePgnsRe.matches("/12345678abcd/black"))
  }

  test("forum links extract game id") {
    assert(group(re.forumGamePgnsRe, "/1234abcd", 2).contains("1234abcd"))
  }

  test("forum links extract game id with color and ply") {
    assert(
      group(re.forumGamePgnsRe, "https://boo.org:8080/abcd1234/black#12", 2).contains("abcd1234")
    )
  }

  test("forum links extract game id with full id and ply") {
    assert(group(re.forumGamePgnsRe, "boo.org:8080/abcd1234wxyz#44", 2).contains("abcd1234"))
  }

  test("forum links match chapter id") {
    assert(re.forumChapterPgnsRe.matches("boo.org:8080/study/abcd1234/chapid33"))
  }

  test("forum links extract chapter id") {
    assert(group(re.forumChapterPgnsRe, "boo.org:8080/study/abcd1234/chapid33", 1).contains("chapid33"))
  }

  test("forum links extract chapter id with ply") {
    assert(group(re.forumChapterPgnsRe, "boo.org:8080/study/abcd1234/chapid33#22", 1).contains("chapid33"))
  }
