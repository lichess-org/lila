...
final class OAuthAppApi(appColl: Coll) {
  ...
  def list(u: User): Fu[List[OAuthApp]] =
    appColl.ext.find($doc(F.author -> u.id)).sort($sort desc F.createdAt).list[OAuthApp](30)

  def create(app: OAuthApp) = appColl.insert.one(app).void
  ...
  def update(from: OAuthApp)(f: OAuthApp => OAuthApp): Fu[OAuthApp] = {
    ...
    else appColl.update.one($doc(F.clientId -> app.clientId), app) inject app
  }
