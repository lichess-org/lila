...
object Protocol {
  ...
  case class RedirectUri(value: AbsoluteUrl) extends AnyVal {
    def appOrigin =
      if (value.hostOption.map(_.value).has("localhost") && List("http", "ionic", "capacitor").has(value.scheme))
        "localhost"
    ...
    def error(error: Error, state: Option[State]): String = value.withQueryString(
      "error" -> Some(error.error),
      "error_description" -> Some(error.description),
      "state" -> state.map(_.value)
    ).toString

    def code(code: AuthorizationCode, state: Option[State]): String = value.withQueryString(
  ...
  object Error {
    ...
    case object UnsupportedCodeChallengeMethod extends UnauthorizedClient("supports only code_challenge_method 'S256'")

    abstract class InvalidGrant(val description: String) extends Error("invalid_grant")
    case object AuthorizationCodeInvalid extends InvalidGrant("authorization code invalid, expired or consumed")
