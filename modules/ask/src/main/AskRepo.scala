...
final class AskRepo(askDb: lila.db.AsyncColl, cacheApi: lila.memo.CacheApi)(using Executor, Scheduler)
  ...
  def unsetPicks(askId: AskId, voterId: String): Fu[Option[Ask]] =
    update(askId, voterId, none[Unit], unsetCached, writeUnset)

  def delete(askId: AskId): Funit = askDb: coll =>
    coll.delete.one(bid(askId)).void.addEffect(_ => cacheRemove(askId))

  def conclude(askId: AskId): Fu[Option[Ask]] = askDb: coll =>
    coll
      .findAndUpdateSimplified[Ask](bid(askId), addToSet("tags" -> "concluded"), fetchNewObject = true)
      .collect:
        case Some(ask) =>
          cachePut(ask)
  ...
  def reset(askId: AskId): Fu[Option[Ask]] = askDb: coll =>
    coll
      .findAndUpdateSimplified[Ask](
        bid(askId),
        bdoc(unset("picks", "form"), pull("tags" -> "concluded")),
        fetchNewObject = true
      )
      .collect:
        case Some(ask) =>
  ...
  def setUrl(encoded: String, url: Option[String]): Funit = askDb: coll =>
    if !AskApi.hasAskId(encoded) then funit
      ...
      coll.update.one(selector, bset("url" -> url), multi = true) >>
  ...
  private def writePicks(askId: AskId, voterId: String, picks: Option[Vector[Int]], fetchNew: Boolean) =
    updateAsk(askId, picks.fold(unset(s"picks.$voterId"))(r => bset(s"picks.$voterId" -> r)), fetchNew)

  private def writeForm(askId: AskId, voterId: String, form: Option[String], fetchNew: Boolean) =
    updateAsk(askId, form.fold(unset(s"form.$voterId"))(f => bset(s"form.$voterId" -> f)), fetchNew)
  ...
  private def updateAsk(askId: AskId, update: BSONDocument, fetchNew: Boolean) = askDb: coll =>
    coll.update
      .one(and(bid(askId), bdoc("tags" -> neq("concluded"))), update)
      .flatMap:
    ...
    coll
      ...
      .flatMap:
        case Some(dbAsk) =>
          ...
          else coll.update.one(bid(ask._id), mergedAsk).inject(mergedAsk).addEffect(cachePut)
        case _ =>
