...
final class FormUi(helpers: Helpers, bits: RelayBits)(
    thumbnailImg: (Option[ImageId], RelayTour.thumbnail.SizeSelector) => Tag
):
  ...
  object round:
    ...
    def inner(form: Form[RelayRoundForm.Data], url: play.api.mvc.Call, t: RelayTour, create: Boolean)(using
        ctx: Context
    ) =
  ...
  object tour:
    ...
    def inner(form: Form[RelayTourForm.Data], tg: Option[RelayTour.WithGroupTours])(using Context) = frag(
  ...
  Team Dogs ; Scooby Doo"""),
          ...
          frag(
            ...
            tg.map: t =>
              div(
                ...
                thumbnailImg(t.tour.pinnedStreamerImage, _.Size.Small16x9)(
                  cls := List(
    ...
    private def image(t: RelayTour)(using ctx: Context) =
      div(cls := "relay-image-edit", data("post-url") := routes.RelayTour.image(t.id))(
        thumbnailImg(t.image, _.Size.Small)(
