...
final private class RelaySync(
  ...
  private def updateChapter(
    ...
    _ = if nbMoves > 0 then notifier.onUpdate(rt, newTags.foldLeft(chapter)(_.withTags(_)), game)
      ...
      .flatMap: nb =>
        (RelayFetch.maxChaptersToShow > nb).so:
          createChapter(study, game)(using rt.tour).map: chapter =>
            if chapter.root.mainline.nonEmpty then notifier.onCreate(rt, chapter, game)
