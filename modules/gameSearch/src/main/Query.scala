...
object Query:
  ...
  def winnerColors(using Translate) = List(1 -> trans.white.txt(), 2 -> trans.black.txt())

  val sources = lila.game.Source.searchable.map { v =>
    v.id -> v.name.capitalize
  }

  def modes(using Translate) = List(0 -> trans.casual.txt(), 1 -> trans.rated.txt())
  ...
  def hasAis(using Translate) = List(0 -> trans.human.txt(), 1 -> trans.computer.txt())
