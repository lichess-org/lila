...
final private class NotificationRepo(val coll: Coll)(implicit
  ...
  private def unreadOnlyQuery(userId: Notification.Notifies) =
    $doc("notifies" -> userId, "read" -> false)
