...
class ReaderTest extends ChessTest {
  ...
  "only raw moves" should {
    "many games" in {
      forall(raws) { (c: String) =>
        Reader.full(c) must beSuccess.like { case Complete(replay) =>
          replay.moves must have size (c.split(' ').size)
        }
      }
    }
    ...
    "and delimiters" in {
      Reader.full(withDelimiters) must beSuccess.like { case Complete(replay) =>
        replay.moves must have size 33
      }
    }
    "and delimiters on new lines" in {
      Reader.full(withDelimitersOnNewLines) must beSuccess.like { case Complete(replay) =>
        replay.moves must have size 33
      }
    }
  }
  "tags and moves" should {
    ...
    "invalid variant" in {
      Reader.full(invalidVariant) must beSuccess.like { case Complete(replay) =>
        replay.setup.board.variant must_== variant.Standard
      }
    }
    "promoting to a rook" in {
      Reader.full(fromLichessBadPromotion) must beSuccess.like { case Complete(replay) =>
        replay.chronoMoves lift 10 must beSome.like { case move =>
          move.fold(_.promotion, _ => None) must_== Some(Rook)
        }
      }
    }
    "chessbase arrows" in {
    ...
    "crazyhouse 1" in {
      Reader.full(crazyhouse1) must beSuccess.like { case Complete(replay) =>
    ...
    "crazyhouse 2" in {
      Reader.full(crazyhouse2) must beSuccess.like { case Complete(replay) =>
        replay.chronoMoves.size must_== 111
      }
    }
    "crazyhouse without variant tag" in {
      Reader.full(crazyhouseNoVariantTag) must beSuccess.like { case Incomplete(replay, _) =>
  ...
  "from prod" in {
    "from position close chess" in {
      Reader.full(fromPosProdCloseChess) must beSuccess.like { case Complete(replay) =>
        replay.chronoMoves.size must_== 152
      }
    }
    "from position empty FEN" in {
      Reader.full(fromPositionEmptyFen) must beSuccess.like { case Complete(replay) =>
        replay.chronoMoves.size must_== 164
      }
    }
    "preserves initial ply" in {
      Reader.full(caissa) must beSuccess.like { case Complete(replay) =>
  ...
  "partial from broadcast" in {
    Reader.full(festivalFigueira) must beSuccess.like { case Incomplete(replay, _) =>
      replay.chronoMoves.size must_== 113
    }
  }
  "invisible char" in {
    Reader.full(invisibleChar) must beSuccess.like { case Complete(replay) =>
