...
case class Assessible(analysed: Analysed) {
  ...
  def mtAvg(color: Color): Int = listAverage(game.moveTimes(color) | List.empty).toInt
  def mtSd(color: Color): Int = listDeviation(game.moveTimes(color) | List.empty).toInt
