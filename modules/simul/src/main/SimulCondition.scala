...
object SimulCondition:
  ...
  trait FlatCond:
    ...
    def apply(user: User, perf: PerfType): SimulCondition.Verdict
  ...
  case object PlayYourGames extends SimulCondition:
  ...
  case object Titled extends SimulCondition with FlatCond:
  ...
  case class NbRatedGame(nb: Int) extends SimulCondition with FlatCond:
  ...
  case class MaxRating(rating: Int) extends SimulCondition:
  ...
  case class MinRating(rating: Int) extends SimulCondition with FlatCond:
  ...
  case class AllowList(value: String) extends SimulCondition with FlatCond:
    ...
    def apply(user: User, @annotation.nowarn perf: PerfType): SimulCondition.Verdict =
  ...
  case class All(
    ...
    lazy val list: List[SimulCondition] =
  ...
  // final class Verify(historyApi: lila.history.HistoryApi, banApi: SimulBanApi):
  //
  //   def apply(simul: Simul, user: User)(using Executor): Fu[All.WithVerdicts] =
  //     val getBan: GetBannedUntil     = banApi.bannedUntil
  //     val getMaxRating: GetMaxRating = perf => historyApi.lastWeekTopRating(user, perf)
  //     simul.settings.conditions.withVerdicts(simul.perfType, getMaxRating, getBan)(user)
