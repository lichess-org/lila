  ...
  def apply(root: Root): Option[NewTree] =
    root.children.first.map: first =>
      NewTree(
        ...
        child = first.children.first.map(fromBranch(_, first.children.variationsOnly)),
        variations = root.children.variationsOnly.map(toVariation)
      )

  def toVariation(branch: Branch): Variation[NewBranch] =
    Variation(
      value = fromBranch(branch),
      child = branch.children.first.map(fromBranch(_, branch.children.variationsOnly))
    )
    ...
    def loop(current: Branch, variations: List[Branch], buildTree: NewTree => NewTree): NewTree =
      ...
      current.children.first match
        ...
        case Some(next) =>
          loop(
            next,
            current.children.variationsOnly,
