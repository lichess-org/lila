...
trait AskApi:
  ...
  def commit(encoded: Encoded, url: Option[String] = none[String]): Fu[Iterable[Ask]]
...
trait AskRepo:
  ...
  def unsetPicks(aid: AskId, vid: String): Fu[Option[Ask]]
...
case class Encoded(text: String, asks: Iterable[Ask])
...
case class Ask(
  ...
  def hasDataFor(o: Option[String]): Boolean =
    hasPickFor(o) || hasFormFor(o)
...
object Ask:
  ...
  val markerIdPrefix = "\ufdd6\ufdd4\ufdd2\ufdd0"
  val markerIdRe = s"$markerIdPrefix\\{(\\S{8})}".r
  ...
  def strip(text: String, n: Int = -1): String =
    markerIdRe.replaceAllIn(text, "").take(if n == -1 then text.length else n)
