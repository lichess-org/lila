...
final class CoachApi(
  ...
  def setSeenAt(user: User): Funit =
    Granter(_.Coach)(user) ?? coachColl.update.one(
      $id(user.id), $set("user.seenAt" -> DateTime.now)
    ).void
      ...
      UserRepo.byId(userPre.id) flatMap {
        _ ?? { user =>
          coachColl.update.one(
            $id(user.id),
            u = $set("user.rating" -> user.perfs.bestStandardRating)
          ).void
        }
      }
    }
  ...
  def update(c: Coach.WithUser, data: CoachProfileForm.Data): Funit =
    coachColl.update.one($id(c.coach.id), data(c.coach), upsert = true).void
  ...
  private[coach] def toggleApproved(username: String, value: Boolean): Fu[String] =
    find(username) flatMap {
      ...
      case Some(c) => coachColl.update.one(
        $id(c.coach.id), $set("approved" -> value)
      ) inject "Done!"
    }

  def remove(userId: User.ID) =
    coachColl.update.one($id(userId), $set("listed" -> false), upsert = false)

  def uploadPicture(c: Coach.WithUser, picture: Photographer.Uploaded): Funit =
    photographer(c.coach.id.value, picture).flatMap { pic =>
      coachColl.update.one(
        $id(c.coach.id), $set("picturePath" -> pic.path)
      ).void
    }
  ...
  object reviews {
    ...
    def approve(r: CoachReview, v: Boolean) = {
      if (v) reviewColl.update.one(
        $id(r.id), $set("approved" -> v) ++ $unset("moddedAt")
      ).void
    ...
    def deleteAllBy(userId: User.ID): Funit = for {
      reviews <- reviewColl.find($doc("userId" -> userId))
        .cursor[CoachReview]().list

      _ <- reviews.map { review =>
        reviewColl.delete(
          ordered = false
        ).one($doc("userId" -> review.userId)).void
    ...
    private def findRecent(selector: Bdoc): Fu[CoachReview.Reviews] =
      reviewColl.find(selector).sort($sort desc "createdAt")
        .cursor[CoachReview]().list(100).map(CoachReview.Reviews.apply)
  }
}
