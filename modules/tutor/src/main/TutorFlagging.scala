...
private object TutorFlagging:
  ...
  def computeIfRelevant(
      user: TutorPlayer
  )(using config: TutorConfig, insightApi: InsightApi, ec: Executor): Fu[TutorFlagging] =
    relevant(user.perfType).so:
      val question =
        Question(InsightDimension.Result, InsightMetric.Termination).filter(Filter(user.perfType))
