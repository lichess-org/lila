...
final class TutorApi(
  ...
  def latestReports(user: User): Fu[List[TutorPeriodReport.Preview]] =
    availability match
  ...
  // NOT for builder
  private def buildThenRemoveFromQueue(query: TutorPeriodReport.Query) =
    val chrono = Chronometer.start
    logger.info(s"Start $query")
    queue.start(query) andDo {
      builder(query).foreach: built =>
        logger.info:
  ...
  private def findLatest(userId: UserId) = colls.report
    .find($doc(TutorPeriodReport.F.user -> userId))
    .sort($sort desc TutorPeriodReport.F.at)
    .list[TutorPeriodReport.Preview](20)
