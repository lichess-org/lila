...
private[report] final class DataForm(val captcher: akka.actor.ActorSelection, val domain: String) extends lidraughts.hub.CaptchedForm {
  ...
  val create = Form(mapping(
    "username" -> lidraughts.user.DataForm.historicalUsernameField.verifying("Unknown username", { fetchUser(_).isDefined }),
