...
case class Report(
  ...
  def userIds: List[UserId] = user :: atoms.toList.map(_.by.value)
...
object Report:
  ...
  private[report] case class SnoozeKey(snoozerId: UserId, reportId: Report.ID) extends lila.memo.Snooze.Key
