...
final class TournamentShow(helpers: Helpers, gathering: GatheringUi)(
  ...
  object side:
    ...
    def apply(
      ...
      frag(
        div(cls := "tour__meta")(
          st.section(cls := "tour__meta__head", iconEl := tour.perfType.icon)(
            div(
              p(
                tour.clock.show,
                separator,
    ...
    def apply(rated: Option[Rated] = None, privateId: Option[TourId] = None)(using Context) =
      frag(
        ...
        table(cls := "slist slist-pad")(
          ...
          tbody(
            tr(
              td(s"${trans.variant.standard.txt()}, ${trans.variant.chess960.txt()}, ${trans.variant.horde.txt()}"),
              td(30)
            ),
            tr(
              td(s"${trans.variant.antichess.txt()}, ${trans.variant.crazyhouse.txt()}, ${trans.variant.kingOfTheHill.txt()}"),
              td(20)
            ),
            tr(
              td(s"${trans.variant.threeCheck.txt()}, ${trans.variant.atomic.txt()}, ${trans.variant.racingKings.txt()}"),
              td(10)
            )
          )
        )
