...
object Condition {
  ...
  case class All(
    ...
    def withVerdicts(
        getMaxRating: GetMaxRating
    )(user: User, getUserTeamIds: User => Fu[List[TeamID]])(
        implicit ec: scala.concurrent.ExecutionContext
    ): Fu[All.WithVerdicts] =
      list.map {
  ...
  final class Verify(historyApi: lila.history.HistoryApi) {
    def apply(all: All, user: User, getUserTeamIds: User => Fu[List[TeamID]])(
        implicit ec: scala.concurrent.ExecutionContext
    ): Fu[All.WithVerdicts] = {
  ...
  object DataForm {
    ...
    val maxRatings = List(2200, 2100, 2000, 1900, 1800, 1700, 1600, 1500, 1400, 1300, 1200, 1100, 1000, 900, 800)
