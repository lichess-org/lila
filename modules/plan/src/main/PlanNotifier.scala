...
import lila.core.actorApi.timeline.{ Atom, Propagate }
...
final private[plan] class PlanNotifier(
  ...
  private def onStart(user: User): Unit =
    ...
    pushTimeline(user)(lila.core.actorApi.timeline.PlanStart.apply)

  private def onRenew(user: User): Unit =
    pushTimeline(user)(lila.core.actorApi.timeline.PlanRenew(_, user.plan.months))
  ...
  def onGift(from: User, to: User, isLifetime: Boolean): Unit =
    pushTimeline(to)(lila.core.actorApi.timeline.PlanStart.apply)
