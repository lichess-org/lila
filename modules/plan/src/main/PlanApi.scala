...
final class PlanApi(
          ...
          userOption ?? { user =>
            ...
            userPatron(user).flatMap {
              case None => patronColl.insert(Patron(
                ...
                setDbUserPlan(user, lidraughts.user.Plan.start) >>
  ...
  def setLifetime(user: User): Funit = UserRepo.setPlan(user, lidraughts.user.Plan(
  ...
  private def addCharge(charge: Charge): Funit =
    chargeColl.insert(charge).void >>- {
      monthlyGoalApi.get foreach { m =>
        ...
        lidraughts.mon.plan.goal(m.goal.value)
        lidraughts.mon.plan.current(m.current.value)
        lidraughts.mon.plan.percent(m.percent)
        if (charge.isPayPal) {
          lidraughts.mon.plan.amount.paypal(charge.cents.value)
          lidraughts.mon.plan.count.paypal()
        } else if (charge.isStripe) {
          lidraughts.mon.plan.amount.stripe(charge.cents.value)
          lidraughts.mon.plan.count.stripe()
        }
      }
    }
