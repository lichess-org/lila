...
private final class MoveDB(
  ...
  def postResult(
    ...
    measurement: lidraughts.mon.Measurement
  ) =
  ...
  private val actor = system.actorOf(Props(new Actor {
    ...
    def receive = {
      ...
      case Mon =>
        ...
        lidraughts.mon.draughtsnet.work.moveDbSize(coll.size)
        lidraughts.mon.draughtsnet.work.queued(key)(coll.count(_._2.nonAcquired))
    ...
    def clearIfFull =
      if (coll.size > maxSize) {
        ...
        lidraughts.mon.draughtsnet.move.dbDrop()
        coll.clear()
      }
  }))
}
