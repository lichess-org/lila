...
final class DraughtsnetApi(
  ...
  def authenticateClient(req: JsonApi.Request, ip: IpAddress): Fu[Try[Client]] = {
    ...
    else repo.getEnabledClient(req.draughtsnet.apikey)
  } map {
    case None => Failure(new Exception("Can't authenticate: invalid key or disabled client"))
    case Some(client) => clientVersion accept req.draughtsnet.version map (_ => client)
  } flatMap {
  ...
  def acquire(client: Client): Fu[Option[JsonApi.Work]] = (client.skill match {
    ...
    .recover {
      case e: lidraughts.hub.Duct.Timeout =>
        ...
        none
      case e: Exception =>
        logger.error(s"[${client.skill}] Draughtsnet.acquire ${e.getMessage}")
        none
    }
  ...
  private def acquireAnalysis(client: Client): Fu[Option[JsonApi.Work]] = sequencer {
    analysisColl.find(
      $doc("acquired" $exists false) ++ {
        ...
        "sender.system" -> 1, // user requests first, then lidraughts auto analysis
  ...
  def postAnalysis(workId: Work.Id, client: Client, data: JsonApi.Request.PostAnalysis): Fu[PostAnalysisResult] =
    repo.getAnalysis(workId).flatMap {
      ...
      case Some(work) =>
        ...
        fufail(NotAcquired)
    }.chronometer.mon(_.draughtsnet.analysis.post)
  ...
  private[draughtsnet] def createClient(userId: Client.UserId, skill: String): Fu[Client] =
  ...
  private[draughtsnet] def setClientSkill(key: Client.Key, skill: String) =
...
object DraughtsnetApi {
  ...
  case object GameNotFound extends LidraughtsException {
    val message = "The game has disappeared"
  }

  case object NotAcquired extends LidraughtsException {
    val message = "The work was distributed to someone else"
  }

  sealed trait PostAnalysisResult
  object PostAnalysisResult {
    case class Complete(analysis: lidraughts.analyse.Analysis) extends PostAnalysisResult
    case class Partial(analysis: lidraughts.analyse.Analysis) extends PostAnalysisResult
