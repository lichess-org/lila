...
final class Mailgun(
  ...
  def send(msg: Mailgun.Message): Funit =
    if (apiUrl.isEmpty) {
      ...
      lidraughts.mon.email.actions.send()
...
${trans.common_contact.literalTo(lang, List("https://lidraughts.org/contact")).render}"""
  ...
  object html {
    ...
    val noteContact = a(itemprop := "url", href := "https://lidraughts.org/contact")(
      span(itemprop := "name")("lidraughts.org/contact")
    )
    ...
    )(span(itemprop := "name")("lidraughts.org"))
