...
package lila.hub.i18n

opaque type I18nKey = String
object I18nKey:
  def apply(key: String): I18nKey            = key
  extension (key: I18nKey) def value: String = key
  import scalatags.Text.RawFrag
  extension (key: I18nKey)(using trans: Translate)
    def txt(args: Any*): String =
      trans.translator.txt.literal(key, args, trans.lang)
    def pluralTxt(count: Count, args: Any*): String =
      trans.translator.txt.plural(key, count, args, trans.lang)
    def pluralSameTxt(count: Long): String = pluralTxt(count, count)
    def apply(args: Matchable*): RawFrag =
      trans.translator.frag.literal(key, args, trans.lang)
    def plural(count: Count, args: Matchable*): RawFrag =
      trans.translator.frag.plural(key, count, args, trans.lang)
    def pluralSame(count: Int): RawFrag = plural(count, count)
