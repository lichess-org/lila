...
final private class FideRepo(
  ...
  object rating:
    ...
    def set(id: FideId, date: YearMonth, elos: Map[FideTC, Elo]): Funit = elos.nonEmpty.so:
      for
        ...
        _ <- ratingColl.update.one(bid(id), updated, upsert = true)
      yield ()
  ...
  object federation:
    ...
    def upsert(fed: Federation): Funit =
      federationColl.update.one(bid(fed.id), fed, upsert = true).void
  ...
  object follower:
    ...
    def follow(u: UserId, p: FideId) = playerColl
      .exists(bid(p))
      .flatMapz:
        followerColl.update.one(bid(followId.make(p, u)), bdoc("u" -> u, "p" -> p), upsert = true).void
    def unfollow(u: UserId, p: FideId) = followerColl.delete.one(bid(followId.make(p, u))).void
    def isFollowing(u: UserId, p: FideId) = followerColl.exists(bid(followId.make(p, u)))

    def count(u: UserId): Fu[Int] = followerColl.countSel(bdoc("u" -> u))
